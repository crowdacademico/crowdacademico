...
import type { TermoUsoResponseAtivo } from '../type/termo-uso.type';
...
// nenhuma.
export async function buscarAtivo(): Promise<TermoUsoResponseAtivo> {
  const resposta = await fetch(`${API_BASE_URL}/termos-uso/ativo`);
  return tratarResposta<TermoUsoResponseAtivo>(resposta);
}
