...
// nele.
function paraBlob(canvas: HTMLCanvasElement, tipoMime: string, qualidade: number): Promise<Blob | null> {
...
interface OpcoesReducao {
  // mesmo perfil (largura/qualidade) usado no backend pro mesmo contexto,
  // ver PERFIL_PROCESSAMENTO_POR_CONTEXTO em arquivo.constants.ts - mantenha
...
// @returns o arquivo reduzido, ou o ORIGINAL se a redução falhar ou não
//   compensar (ficar maior que o original).
export async function reduzirImagemNoNavegador(
  arquivo: File,
  { larguraMaxima, qualidade }: OpcoesReducao,
): Promise<File> {
  ...
  try {
    ...
    // `getContext('2d')` pode devolver null (raro, mas o tipo do DOM exige
    // checar) - cai no catch abaixo como qualquer outra falha, mesmo
    // resultado de sempre (arquivo original, sem bloquear o upload).
