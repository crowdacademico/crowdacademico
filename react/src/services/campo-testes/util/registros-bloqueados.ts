...
export function PESQUISADOR_BLOQUEADO(idUsuario: number): boolean {
  return idUsuario >= PRIMEIRO_ID_PESQUISADOR_BLOQUEADO && idUsuario <= ULTIMO_ID_PESQUISADOR_BLOQUEADO;
}

export function motivoBloqueioPesquisador(): string {
...
export function CAMPANHA_BLOQUEADA(idCampanha: number): boolean {
  return idCampanha <= ULTIMO_ID_CAMPANHA_BLOQUEADA;
}

export function motivoBloqueioCampanha(): string {
