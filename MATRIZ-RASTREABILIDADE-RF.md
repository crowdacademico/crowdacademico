...
📌 **Atualizada em 05-09-2026 contra `informacoes/REQUISITOS_V5.md` (v9 do `.docx`).** Diferente da transição V3→V4 (que mudou a numeração inteira), **V4→V5 manteve os mesmos 116 RFs/17 RNFs, mesma numeração** - a mudança foi só de redação: vários RFs ganharam a frase explícita "parâmetro configurável pelo Administrador, com valor padrão de X", deixando formal o que antes só o código/`PENDENCIAS.md` documentava. Conferi todo "valor padrão" citado em `REQUISITOS_V5.md` contra as chaves de verdade em `configuracoes` (seed) - **bateu em todos, com uma exceção**: ver nota no RF-039, abaixo. Nenhuma outra mudança de status foi necessária nesta rodada - o resto da matriz (símbolos ✅/🟡/❌/➖) continua válido.

📌 **06-09-2026: RF-015A adicionado** (exportação de dados/portabilidade LGPD) - 117 RFs em `REQUISITOS_V5.md` agora, não mais 116. É uma inserção informal (o Lucas ainda não renumerou o `.docx` oficial, onde isso vira RF-016 e empurra o resto) - tratar "RF-015A" como o identificador de trabalho até essa renumeração acontecer.

⚠️ **A numeração dos RFs mudou entre `REQUISITOS_3.md` e `REQUISITOS_V4.md`, não é só um acréscimo no final** (isso NÃO se repete entre V4 e V5, ver nota acima). Um mesmo número podia apontar pra requisitos diferentes nas versões 3 e 4 - por exemplo, "RF-079" na numeração antiga era "encerrar campanha por moderação" (agora **RF-108**); na numeração V4/V5, RF-079 é "valor mínimo de contribuição". Qualquer menção a "RF-XXX" em outro documento do projeto que ainda não foi conferida contra `REQUISITOS_V5.md` deve ser tratada com desconfiança até ser revisada - não presuma que o número antigo ainda vale.
...
- 🆕 **RF-015A** (exportação de dados pessoais, acesso/portabilidade LGPD Art. 18, formato estruturado, exclui dado de terceiro, CPF mascarado, limitado em frequência e auditado) - ADICIONADO em 06-09-2026 (`REQUISITOS_V5.md`, entre RF-015 e RF-016 - renumeração formal pro `.docx` fica pro Lucas, fora de escopo desta matriz) - Banco ✅ (`registrar_exportacao_dados()` - `[03-O]`; `CK_LOG_AUDITORIA_OPERACAO` aceita `'EXPORT'`) · Nest ✅ (`GET /usuario/eu/exportar-dados`, sem `:id` - ator sempre o autenticado; rate limit 1x/hora por conta via `ExportarDadosThrottlerGuard`; `Cache-Control: no-store` - ver `DOCUMENTACAO_BACKEND.md` §7.4).
...
- **RF-039** (orçamento estruturado, soma = meta, mín. na aprovação/máx. no cadastro, configuráveis) - Banco ✅ (`orcamento_campanha`, `fn_valida_completude_campanha_aprovacao`, `fn_valida_limite_max_orcamento_campanha` - `[01-E]`) · Nest ✅ (`13-orcamento-campanha`). 🔁 **Divergência achada e corrigida em 05-09-2026:** `configuracoes.orcamento_min_itens` estava seedado com 3, mas `REQUISITOS_V5.md` diz "valores padrão de 1 (mínimo) e 10 (máximo)" - decisão do Lucas foi ajustar o banco pro texto oficial do requisito (1, não 3). Migração em `ATUALIZAR O SUPABASE.sql`. `cronograma_min_marcos` não foi afetado, RF-041 já citava 3 corretamente.
...
**Cuidado geral, fora do escopo desta matriz:** outros documentos do projeto podem ainda citar número de RF da numeração antiga (pré-V4) por engano - o item 57 de `PENDENCIAS` (que citava "RF-079" pra se referir a encerrar campanha por moderação, na verdade **RF-108** desde a V4) já foi revisado e corrigido, mas não há garantia de que seja o único caso. Qualquer citação de RF por número, em qualquer documento, vale conferir contra `REQUISITOS_V5.md` (numeração idêntica à V4) antes de tratar como certa.
