...
- 🆕 **RF-015A** (exportação de dados pessoais, acesso/portabilidade LGPD Art. 18, formato estruturado, exclui dado de terceiro, CPF mascarado, limitado em frequência e auditado) - ADICIONADO em 06-09-2026, entre RF-015 e RF-016 desta linha (numeração desta matriz, anterior à atualização de requisitos de 06-09-2026 - ⚠️ **já foi promovido a RF-016 de verdade na numeração atual**, empurrando o RF-016/RF-017 logo abaixo pra RF-017/RF-018 - mantido como "RF-015A" aqui só pra não colidir com a linha de baixo, que ainda está na numeração anterior, não-propagada) - Banco ✅ (`registrar_exportacao_dados()` - `[03-O]`; `CK_LOG_AUDITORIA_OPERACAO` aceita `'EXPORT'`) · Nest ✅ (`GET /usuario/eu/exportar-dados`, sem `:id` - ator sempre o autenticado; rate limit 1x/hora por conta via `ExportarDadosThrottlerGuard`; `Cache-Control: no-store` - ver `DOCUMENTACAO_BACKEND.md` §7.4).
...
- **RF-039** (orçamento estruturado, soma = meta, mín. na aprovação/máx. no cadastro, configuráveis) - Banco ✅ (`orcamento_campanha`, `fn_valida_completude_campanha_aprovacao`, `fn_valida_limite_max_orcamento_campanha` - `[01-E]`) · Nest ✅ (`13-orcamento-campanha`). 🔁 **Divergência achada e corrigida em 05-09-2026:** `configuracoes.orcamento_min_itens` estava seedado com 3, mas o requisito (RF-039 na numeração usada nesta matriz; **RF-040 na numeração atual**) diz "valores padrão de 1 (mínimo) e 10 (máximo)" - decisão do Lucas foi ajustar o banco pro texto oficial do requisito (1, não 3). Migração em `ATUALIZAR O SUPABASE.sql`. `cronograma_min_marcos` não foi afetado, RF-041 já citava 3 corretamente.
...
Contagem aproximada por símbolo (RF-001 a RF-116, sem RNFs - ⚠️ **esta seção e os números de RF citados abaixo ainda são da numeração anterior (pré-06-09-2026)** - some 1 a qualquer número ≥016 pra converter pra numeração atual; ver nota no topo deste documento - não foram propagados de propósito, é o Lucas quem vai fazer isso por conta própria):
...
**Cuidado geral, fora do escopo desta matriz:** outros documentos do projeto podem ainda citar número de RF de numeração antiga por engano - o item de `PENDENCIAS` sobre encerrar campanha por moderação (que citava "RF-079" de uma numeração já superada, na verdade **RF-108 antes de 06-09-2026, RF-109 na numeração atual**) já foi revisado e corrigido, mas não há garantia de que seja o único caso, e agora há uma segunda camada (a atualização de 06-09-2026) a considerar. Qualquer citação de RF por número, em qualquer documento (esta matriz inclusive, ver nota no topo), vale conferir contra os Requisitos Funcionais atuais antes de tratar como certa.
