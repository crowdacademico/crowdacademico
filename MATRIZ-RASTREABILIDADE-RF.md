...
- **RNF-003** (LGPD, exclusão a pedido, retenção legal) - ✅/🟡 (banco muito forte: `excluir_conta_usuario`, `cpf_criptografado`; ver também RNF-015 e o item 59 de `PENDENCIAS.md`, pendência real de base legal documentada pro score público). 🟢 **Nest ganhou também `GET /usuario/eu/exportar-dados` (05-09-2026, portabilidade, LGPD Art. 18)** - ver `DOCUMENTACAO_BACKEND.md` §7.4. **Achado à parte, não bloqueante:** o texto de RNF-003 em `REQUISITOS_V5.md` fala só em "exclusão a pedido do titular" - não menciona portabilidade/exportação explicitamente em nenhum RF/RNF. A implementação está mais completa que o texto formal do requisito; não há RF numerado pra apontar aqui como "atendido" - só a nota desta linha. Vale decidir, quando o Lucas tiver tempo, se isso vira um RF/RNF novo formal ou fica só como excedente documentado (não é urgente - já registrado, não custa esperar).
