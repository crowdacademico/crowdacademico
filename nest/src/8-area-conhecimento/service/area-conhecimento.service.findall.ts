...
export class AreaConhecimentoServiceFindAll {
  ...
  async executar(
    ...
    let query = this.database
      ...
      // Ordena por id (catálogos ordenam por ID, sempre). O seed insere as 9 grandes áreas primeiro (1.00-9.00,
      // viram id 1-9) e depois cada leva de filhas logo após a grande área correspondente: todas as 9 grandes
      // áreas aparecem primeiro (id 1-9), e só depois vêm todas as filhas, agrupadas por leva de inserção (as 8
      // filhas de área 1, depois as 13 de área 2, etc.); não interlaça
      // grande-área/filha/filha/próxima-grande-área como ordenar por código faria. Quem quer só as filhas de
      // uma área usa o filtro `idPai` (já existe, filtrosFacetados no front).
