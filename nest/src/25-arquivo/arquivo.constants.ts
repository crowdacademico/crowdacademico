...
// Os tipos que passam pelo sharp (tudo menos PDF), derivados da lista mestra acima.
export const TIPOS_IMAGEM_PERMITIDOS: readonly TipoMimePermitido[] =
  TIPOS_MIME_PERMITIDOS.filter((tipo) => tipo !== 'application/pdf');
...
// Tetos de sanidade por tipo: DEFAULT, usado só quando a chave correspondente não existir/estiver inativa em
// `configuracoes` (ver ConfiguracaoValorService, commons/configuracao). Configurável pelo Painel Admin: mesmo
// espírito de tudo que saiu de hardcoded para `configuracoes` (prazo de campanha, limite de denúncia etc.).
//
// Os valores abaixo são só o PONTO DE PARTIDA (8MB imagem / 5MB documento); o valor de VERDADE, em produção, é
// o que estiver em `configuracoes` (ver seed, chaves
// ARQUIVO_TAMANHO_MAXIMO_IMAGEM_BYTES/ARQUIVO_TAMANHO_MAXIMO_DOCUMENTO_BYTES).
...
// Piso de sanidade: rejeita arquivo vazio/quase vazio cedo, com mensagem clara, em vez de deixar o `sharp` (ou
// o próprio provedor de armazenamento) falhar de um jeito confuso mais adiante no fluxo para um arquivo
...
// Cota de armazenamento por usuário: nenhum teto POR ARQUIVO protege contra alguém subindo mil arquivos
// pequenos (com espaço total limitado, 20 contas maliciosas em 50MB cada já tomariam a cota inteira). Somado
// contra `arquivo.tamanho_bytes` (coluna `id_usuario_upload`, ver arquivo.service.confirmar-upload.ts).
// DEFAULT, configurável via `configuracoes` (ver comentário do teto de tamanho acima).
...
// Rate limit de upload: duas proteções complementares, mesmo espírito de
// `limite_tentativas_login`/`bloqueio_login_minutos` (banco, [03-O]) e `limite_denuncias_24h`:
// 1. Quantidade máxima de uploads CONFIRMADOS dentro de uma janela de tempo (evita um usuário legítimo mas
// descuidado, ou um script, enchendo a conta de arquivos rápido demais; complementar à cota de bytes, que
// sozinha não impede MUITOS arquivos pequenos).
// 2. Intervalo mínimo entre um upload confirmado e o próximo início de upload: barra rajada (ex.: um script
// chamando iniciar-upload em loop), sem incomodar o uso humano normal (uma pessoa nunca sobe dois arquivos com
// menos de alguns segundos de diferença).
...
// Contextos de upload: cada um usa um teto de redimensionamento/qualidade diferente em confirmar-upload.ts,
// porque nenhuma tela do sistema mostra avatar maior que uns 96-128px de verdade (512px já é folga generosa
// para tela retina) enquanto uma capa de campanha em destaque pode ocupar a largura inteira de um monitor
// comum. Lista FECHADA de propósito, mesmo espírito de TIPOS_MIME_PERMITIDOS: um contexto novo precisa de
// decisão de produto (qual teto?), nunca deveria "vazar" um valor arbitrário vindo do cliente.
...
// Nenhuma tela do protótipo de interface usa avatar maior que 512px nem imagem de campanha/anexo de atualização
// maior que 1600px de largura útil, mesmo em tela cheia num monitor comum. Qualidade WebP 78-80 é visualmente
// quase indistinguível do original para exibição em tela.
