...
// O mesmo bloco de 5 linhas se repetia em ~27 services: um UPDATE/DELETE que afetou 0 linhas pode ser "não
// existe" OU "existe mas a RLS bloqueou" (dono/permissão faltando), e só um SELECT à parte (sempre visível,
// pol_*_select costuma ser mais aberta que pol_*_update/delete) distingue os dois; sem isso, todo mundo sem
// permissão veria 404 (vaza que existe) em vez do 403 correto.
//
// `sql.table`/`sql.ref` (não string interpolada direto): identificador escapado corretamente pelo Kysely, mesmo
// sem risco real de injeção aqui (tabela/coluna são sempre literais escritos no código, nunca entrada do
// usuário). Via `sql` em vez do query builder tipado porque os genéricos do Kysely (`selectFrom<TB>`) não
// resolvem bem quando TB é um type parameter abstrato, não uma união de literais.
//
// `filtro` é um objeto coluna -> valor: chave simples ({ id_campanha: id }), chave composta ({ id_usuario,
