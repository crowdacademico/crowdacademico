...
- **Campos travados no Alterar Campanha (26-09-2026):** o modal busca `GET /campanha/:id` ao abrir, em qualquer status, e desabilita título, área, descrição, vídeo, datas e meta conforme `camposBloqueados` (a lista do banco, ver `[05-K-2-D]` em `DOCUMENTACAO_BD.md`). Um aviso visível ("Campos travados: depois da aprovação estes campos não mudam, para proteger quem já contribuiu") nomeia os campos e cada campo travado aponta para ele por `aria-describedby`; a campanha rejeitada sem reenvios continua com o aviso próprio e todos os campos desabilitados. A tela real de "Minhas campanhas" deve reaproveitar a mesma ideia.
