...
📌 **Contraste WCAG AA (24-09-2026, medido no Playwright e recalculado a partir do `1-cores.css`).** Texto normal pede 4,5:1. Reprovavam, no tema claro: botão principal (branco sobre o verde da marca `#0f9b58`, 3,59), `--cor-fundo-sucesso-forte` (emerald-600, 3,77, o "3,77" da matriz de permissões), `badge-sucesso` (3,32), `badge-aviso` (2,86, o pior e o único que ninguém tinha medido), `badge-erro` (3,95), `badge-info` (4,24), texto e link na cor da marca sobre branco (3,59), três avatares com inicial branca (3,30 a 3,74) e o placeholder (cerca de 2,6). No tema escuro só a cor da marca como texto sobre o cartão reprovava (4,07).
...
📌 **`LIMITE_ENDOSSOS` deixou de ser constante (24-09-2026).** `vida-campanha-ativa.tsx` (T3) tinha `const LIMITE_ENDOSSOS = 4` escrito à mão, enquanto o limite real é `configuracoes.limite_endossos_campanha` (chave pública). Agora lê por `useConfiguracoes().obterConfiguracao('limite_endossos_campanha', 4)`, o mesmo padrão de `bancada-campanha.tsx`; o `4` sobra só como reserva enquanto a configuração carrega.

📌 **`SENHA_DEV` e o botão "Redefinir senha dev" não vão mais para o pacote de produção (24-09-2026).** A senha de teste (`DevTcc123!`) e o cartão `<dev>` do modal de Alterar Usuário estavam no `dist` de produção, porque `modal-usuario.tsx` é código real, usado na tela de Usuários de verdade. Agora `SENHA_DEV = import.meta.env.DEV ? 'DevTcc123!' : ''` e o cartão só renderiza dentro de `{import.meta.env.DEV && ( ... )}`. `import.meta.env.DEV` é uma constante embutida do Vite (verdadeira em `npm run dev`, falsa em `npm run build`), **não é lida de nenhum `.env`**: nenhuma variável nova, nenhum `.env` alterado. Conferido no `dist`: com `npm run build`, zero ocorrências de `DevTcc123` e do texto do botão; com um build em modo desenvolvimento, as duas presentes. **`registros-bloqueados.ts` não precisou de mudança:** só o importam telas do Campo de Testes (`bancada-campanha`, `bancada-pesquisador`), cujas rotas já são criadas apenas com `import.meta.env.DEV`; o Vite descarta essas telas do build e os textos do arquivo (ex.: "Pesquisador de demonstração") têm zero ocorrências no `dist` de produção. Efeito para a Alexia: nenhum, se ela roda com `npm run dev`; só o botão de redefinir senha some se ela usar um build de produção.

📌 **Alerta de mínimo e máximo no modal de Alterar Parâmetro (24-09-2026).** Quando a chave editada faz parte de um par mínimo/máximo que o banco confere (`fn_valida_pares_min_max_configuracoes`, erro 90019: prazo, orçamento, cronograma e tamanho de arquivo), o modal mostra uma caixa de aviso amarela dizendo se aquele valor é o MÍNIMO ou o MÁXIMO, com qual chave ele precisa se manter coerente e em que ordem editar (para subir o mínimo acima do máximo atual, suba o máximo primeiro, e o contrário para baixar). O aviso vem de `services/11-configuracoes/constants/configuracao-pares-min-max.ts`, que **só espelha** os pares para explicar antes de salvar; a regra de verdade é a do banco, que continua recusando com a mensagem própria (exibida no mesmo modal). Se um par novo entrar no banco, entra nessa lista também.

📌 **`npm run contraste` (24-09-2026).** `react/scripts/contraste-tokens.mjs` lê os tokens de `1-cores.css` e confere o contraste WCAG AA (4,5:1) de 20 pares nos dois temas (texto sobre cartão, texto de estado sobre o fundo do badge, texto branco sobre os fundos sólidos e sobre os 7 avatares). Sai com código 1 se algum par ficar abaixo. **Foi provado que ele falha quando deve:** numa cópia, trocar `--cor-fundo-marca-forte` de volta para o verde da marca reproduz exatamente o 3,59:1 original. Limites: só mede cor sólida (hex, `rgba` sobre o cartão, `var()`); `color-mix()` aparece como "não medido"; não enxerga herança, opacidade nem gradiente, então não substitui olhar a tela nem uma auditoria com axe. Ainda não está ligado a nenhum passo automático (rodar antes de mexer em `1-cores.css`).

📌 **`TAMANHO_PAGINA_MAXIMO_API` (24-09-2026).** O `500` (teto que o backend aplica em qualquer listagem, `paginacao.util.ts`) estava escrito à mão em `campanha.api.ts` e `perfil-pesquisador.api.ts`. Agora é uma constante exportada de `services/constant/type/paginacao.type.ts`, ao lado de `desembrulharPaginado`.
...
📌 **F5 em T2 e T3 (24-09-2026, achado no Playwright).** Os efeitos que carregam as listas ao montar (`bancada-campanha.tsx` e `vida-campanha-ativa.tsx`) disparavam antes de a sessão ser restaurada: com recarga completa o `authFetch` ainda não tinha token e a lista de campanhas vinha só com as públicas (10 em vez de 20); navegando pelo menu vinha completa. Agora os efeitos esperam `auth.carregando` ser `false` (`if (auth.carregando) return`, e `auth.carregando` nas dependências). T1 e T4 não foram testadas com F5.

📌 **Texto de Excluir campanha (24-09-2026).** O modal de Excluir dizia que só dá para excluir campanha "aguardando aprovação"; a regra real (`pol_campanha_delete`, `statusNaoElegivel`) é só `rascunho`. Texto corrigido.
...
- **`baixo`** - abre a dica pra BAIXO em vez de pra cima (padrão). **Todo controle do cabeçalho (A-, A+, tema, sino) usa `baixo` (24-09-2026, achado no Playwright):** o cabeçalho fica colado no topo da janela, e a bolha padrão (pra cima) nascia a y = -20px, fora da tela. Usar quando o ícone fica perto do topo de um cartão com `overflow-hidden` (ex.: cabeçalho de grupo em `dashboard-regras-negocio.tsx`) - a dica padrão nascia cortada pela borda arredondada do cartão.
