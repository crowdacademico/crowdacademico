...
- **Seções:** (1) comparador do verde do texto no tema escuro (candidatos sobre o cartão e sobre a página, com a razão de contraste e um seletor de cor para testar outra); (2) cores do sistema, com cada par texto/fundo e a razão **medida no navegador** (`services/campo-testes/util/contraste-cor.util.ts` pinta 1 pixel em canvas para normalizar qualquer cor, inclusive `color-mix()`, e compõe a transparência sobre o fundo), mais amostras de superfícies, marca, bordas e fundos de estado; (3) tipografia (as 9 classes de `2-tipografia.css`); (4) componentes (botões, badges, campos normal, com erro e desabilitado, avisos e cartões).
- **Uma lista só de pares de contraste:** `services/campo-testes/constants/pares-contraste.json`, lida pela página (import do JSON) e pelo `npm run contraste` (`fs`). Se um par entrar, entra nos dois.
