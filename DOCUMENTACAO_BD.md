...
* **As 5 funções:**
  ...
  |---|---|---|
  | `confirmar_email_usuario(p_id_usuario)` | `email_verificado = TRUE` | Chamada depois do NestJS validar o token em `verificacao_email` — não valida o token, só aplica o resultado. |
  | `registrar_falha_login(p_id_usuario)` | `tentativas_login_falhas++`, `bloqueado_ate` condicional | Bloqueia ao atingir `configuracoes.limite_tentativas_login` (5), por `configuracoes.bloqueio_login_minutos` (15) — nenhum número fixo, os dois configuráveis pelo Painel Admin. |
  | `liberar_bloqueio_login(p_id_usuario)` | `tentativas_login_falhas = 0`, `bloqueado_ate = NULL` | Desbloqueio manual (suporte/admin), antes do próximo login bem sucedido. |
  | `registrar_login_sucesso(p_id_usuario, p_ip)` | `ultimo_login_em`, `ultimo_login_ip`, e zera o estado de falha | Um login bem sucedido sempre limpa o histórico de tentativas anteriores — `p_ip` é `TEXT` na assinatura (não `VARCHAR(45)`, o tipo da coluna) só pra evitar ambiguidade de modificador de tipo no `GRANT EXECUTE`. |
  | `excluir_conta_usuario(p_id_usuario)` | `deletado = TRUE` | RNF-003 (LGPD). Via de mão única, de propósito — não existe função equivalente pra reverter, coerente com o desenho de anonimização já existente (`usuario_visivel()`, `[03-D]`). |
