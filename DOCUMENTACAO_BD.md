...
> ⚠️ **Colisão pré-existente não corrigida, achada em 09-08-2026:** `[03-F]` ("OPERAÇÕES DE AUTENTICAÇÃO") e `[03-G]` ("MODERAÇÃO SOBRE PESQUISADOR") reaproveitam as letras `F`/`G` com um significado DIFERENTE do que esta tabela atribui a elas (`F`=LINK, `G`=ARQUIVO) — mesma classe de bug já corrigida uma vez (`[03-K]`→`[03-M]`), só que mais antiga e ainda em aberto. Não corrigida agora (é um rename maior, cruzando comentários e referências já estabelecidas — fora do escopo da rodada que achou o problema); por isso o bloco de moderação de CONTA desta rodada ganhou a letra `N` (livre), não `F`/`G`, pra não empilhar uma 3ª colisão em cima das duas já existentes. Registrado como pendência em `PENDENCIAS e correcoes.md`.

> 📌 Por que `J`, `K`, `M` e `N` existem: nem tudo no banco pertence a um domínio de dado único. `id_usuario_atual()` (`03`) não é sobre nenhuma tabela específica — é infraestrutura de sessão usada por tudo. E várias triggers de `05` (ex.: validar repasse financeiro de uma campanha, ou impedir denúncia excessiva) mexem em mais de uma tabela de domínios diferentes ao mesmo tempo — forçá-las dentro de uma letra só (`E` ou `H`, por exemplo) esconderia que elas são regras de fronteira entre domínios, não de um domínio só. Por isso ganham letras próprias em vez de reaproveitar `A`-`I` com um significado diferente do já estabelecido. `L` (log de auditoria, 03-08-2026) segue a mesma lógica: não pertence a nenhum domínio específico (a mesma trigger genérica se aplica a `usuario`, `perfil_pesquisador`, `configuracoes`, RBAC e vários catálogos ao mesmo tempo). `M` (dashboard admin, 08-08-2026, corrigido de `K` pra `M` em 09-08-2026 depois de um auditoria encontrar que `K` já estava em uso por `05-K`) é a mesma ideia: `contar_metricas_dashboard()` agrega número de várias tabelas de domínios diferentes numa função só, não pertence a nenhuma sozinha. `N` (moderação de conta, 09-08-2026) é diferente das outras — não é "cross-domain" como J/K/M, é USUÁRIO de verdade (podia ter sido `D`), só ganhou letra própria pra não virar a 3ª colisão em cima de `F`/`G` já existentes (ver aviso acima) — assim que a colisão `F`/`G` for corrigida algum dia, vale reavaliar se `N` devia ter sido `D` desde o início.
...
* **4 funções novas, todas `SECURITY DEFINER`:** `suspender_usuario(p_id_usuario, p_ate, p_motivo)`/`revogar_suspensao_usuario(p_id_usuario)` exigem `usuario_suspender` (mesma permissão de `suspender_pesquisador`, `[03-G]`); `suspender_papel_usuario(p_id_usuario, p_id_papel, p_ate)`/`revogar_suspensao_papel_usuario(p_id_usuario, p_id_papel)` exigem `papel_gerenciar` (é decisão de RBAC, não de moderação de conta). "Reduzir a pena" não tem função própria — é chamar `suspender_usuario`/`suspender_papel_usuario` de novo com uma data mais próxima, a função já sobrescreve.
...
* **Letra `N`, não `F`/`G`:** ver aviso no Índice Global de Letras (topo deste documento) sobre a colisão pré-existente de `[03-F]`/`[03-G]`.
