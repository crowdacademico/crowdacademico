...
- 🟢 **Exportação de dados do usuário - RESOLVIDO (05-09-2026), migração já rodada em produção.** `GET /usuario/eu/exportar-dados` (novo, dentro de `1-usuario`, mesmo padrão de `usuario.service.remove.ts`) - sem `:id`, ator sempre o autenticado. CPF sai mascarado (nunca em texto puro); rate limit de 1x/hora por conta (não por IP); rastro em `log_auditoria` a cada chamada (`registrar_exportacao_dados()`, `SECURITY DEFINER`); `Cache-Control: no-store`. Detalhamento completo em `DOCUMENTACAO_BACKEND.md` §7.4 e `DOCUMENTACAO_BD.md` (`[03-O]` + seção `log_auditoria`). Formalizado como **RF-016** nos Requisitos Funcionais, atualizados em 06-09-2026.
