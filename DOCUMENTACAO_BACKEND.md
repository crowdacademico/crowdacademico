...
📌 **Baixados de 10MB/20MB (01-09-2026).** O motivo está no comentário: o projeto roda no plano grátis do Supabase Storage, com **1 GB de espaço total** — um teto de 20MB por PDF deixava um único upload malicioso ocupar 2% da cota inteira. A imagem cai menos porque 8MB é folga de sobra para foto de celular sem tratar, e o processamento reduz o que sobra para uma fração disso.
...
**O problema que ele resolve:** os 8 arquivos de `arquivos_banco_dados/*.sql` são colados à mão no SQL Editor do Supabase, **sem nenhum registro de qual arquivo já rodou em qual banco**. Com Lucas e Alexia em bancos Supabase separados, não havia garantia de que os dois estavam no mesmo estado.
...
13. ⚠️ **`ARQUIVO - Dica do CLAUDE.md` cita Cloudflare R2** como provedor; o atual é Supabase Storage. O desenho continua válido, só o nome mudou. (§8.1)
...
| `ARQUIVO - Dica do CLAUDE.md` | O "doc de arquitetura" citado nos comentários de `commons/storage` e `25-arquivo` |
