...
Existe agora um script que resolve isso - `src/commons/database/aplicar-migrations.script.ts`, rodado via `npm run db:migrate` (dentro de `nest/`). Ele lê os 8 arquivos na ordem certa, guarda um registro (tabela `schema_migrations`, criada sozinha) de quais já rodaram, e só aplica o que for novo. Detalhe técnico completo (o que cada modo faz, por que `schema_migrations` não é um dos 8 arquivos numerados) em `DOCUMENTACAO_BACKEND.md`, seção 12.
...
1. No `.env` do backend, adicione uma SEGUNDA linha de conexão, além da `DATABASE_URL` normal:
   ...
   📌 **Não sabe/não lembra a senha do `postgres` do seu projeto Supabase?** Ela **não é** `postgres123` nem `app_nestjs123` - essas duas são só convenção deste tutorial pro Postgres **local** (Partes 1-3); a senha do `postgres` de um projeto Supabase específico foi escolhida (ou gerada) uma única vez, quando você criou aquele projeto, e o Supabase não consegue "mostrar ela de volta" depois - só resetar. Pra resetar: no painel do projeto, vá em **Project Settings → Database**, procure a seção de connection string e clique em **"Reset database password"**. Ele mostra a senha nova em texto puro **só até você atualizar a página** ("New password shown until refresh") - anote em algum lugar seguro (gerenciador de senha, ou um bloco de notas fora do repositório) antes de sair da tela. **Resetar a senha do `postgres` não afeta o `app_nestjs` de jeito nenhum** - são dois usuários (roles) completamente separados dentro do mesmo Postgres, cada um com sua própria senha guardada à parte; o `app_nestjs` nem foi criado pelo Supabase, foi criado pelo nosso próprio `01_extensoes_enums_tabelas.sql`. Se quiser, pode usar a mesma senha nos dois por praticidade (ex.: `app_nestjs123` nos dois) - só vale lembrar que isso mistura um usuário sem privilégio nenhum com o superusuário que pode fazer qualquer coisa no banco, então não é recomendado se este projeto um dia virar produção de verdade, com dado real.

   📌 **Use a Direct Connection (porta `5432`, host `db.xxxxxxxxxxxx.supabase.co`), não o Shared Pooler (porta `6543`, host `...pooler.supabase.com`).** O Supabase mostra os dois na tela de conexão. O script de migration manda cada `.sql` inteiro numa única mensagem, com várias instruções separadas por `;` (necessário pra rodar função/trigger com corpo grande em `$$...$$`) - o Pooler compartilhado normalmente roda num modo que não lida bem com isso numa conexão dividida entre vários clientes. A Direct Connection é uma conexão só sua, sem esse problema - é o modo certo pra esse tipo de script administrativo (rodado de vez em quando, não o tráfego constante do app).

   📌 **Se a senha tiver caractere especial** (`@`, `/`, `#`, etc. - comum em senha gerada automaticamente pelo botão de reset), a própria tela do Supabase avisa: é preciso "percent-encode" (trocar por código `%XX`) antes de colar na URL, senão ela fica malformada. Mais simples: ao definir a senha, se o Supabase deixar escolher a sua em vez de usar a gerada, prefira uma só com letra e número.
...
### ⚠️ Problemas comuns rodando `npm run db:migrate`/`:adotar`

- **`npm : O arquivo ...\npm.ps1 não pode ser carregado porque a execução de scripts foi desabilitada neste sistema` (PowerShell).** Não é bug do projeto - é uma trava padrão do Windows: o PowerShell recusa rodar qualquer script `.ps1` por padrão, e o `npm` é, por baixo, um desses scripts. Solução mais simples: rode o comando no **`cmd`** (Prompt de Comando) ou no **Git Bash** em vez do PowerShell - os dois usam `npm.cmd`, que não passa por essa trava. Não precisa mexer em nenhuma configuração de segurança do Windows.
- **`error: password authentication failed for user "postgres"` (código `28P01`).** A conexão chegou certinha no Supabase e a senha foi recusada - ou seja, a `DATABASE_URL_MIGRATIONS` do seu `.env` tem uma senha diferente da que o Supabase tem guardada de verdade pro `postgres` daquele projeto. Resolve resetando a senha (ver 📌 acima) e atualizando a linha no `.env` com o valor novo.
- **`◇ injected env (N) from .env // tip: ...` (aparece toda vez, antes de qualquer coisa).** Não é erro, não é nosso código - é um aviso/dica promocional do próprio pacote `dotenv` (versão 17), que muda de mensagem a cada execução. Só ignorar; o número entre parênteses é quantas variáveis ele leu do `.env` (útil só pra confirmar que uma linha nova foi mesmo lida - se você acabou de adicionar uma variável e o número não aumentou, o arquivo não foi salvo ou está no lugar errado).
- **Aparece um 9º arquivo na lista do `--adotar`, `ATUALIZAR O SUPABASE.sql` (achado e corrigido em 05-09-2026).** Versões antigas do script listavam **qualquer** `.sql` dentro de `arquivos_banco_dados/`, não só os 8 numerados - e `ATUALIZAR O SUPABASE.sql` (o rascunho vivo de ajustes pequenos, que cresce toda semana) mora na mesma pasta. Isso foi corrigido: o script agora só aceita nomes que começam com exatamente dois dígitos e `_` (`01_` a `08_`) - `ATUALIZAR O SUPABASE.sql` nunca mais aparece. Se você rodou `--adotar` **antes** dessa correção, sobrou uma linha órfã pra esse arquivo em `schema_migrations` - inofensiva (o script não procura mais por esse nome), pode ignorar ou apagar com `DELETE FROM schema_migrations WHERE nome_arquivo = 'ATUALIZAR O SUPABASE.sql';` no SQL Editor, se quiser deixar a tabela "limpa".
...
Isso não foi testado ao vivo em todo ambiente (alguns não têm Postgres nem Docker disponível), mas já foi testado ao vivo com Postgres de verdade disponível: 15 requisições concorrentes de cada usuário contra a mesma rota protegida, isolamento perfeito (ver `PENDENCIAS e correcoes.md`, item 5). Ainda assim, vale repetir aqui na sua máquina antes de confiar de olhos fechados - ambiente diferente, sempre bom conferir de novo.
