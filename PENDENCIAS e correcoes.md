...
> **(1) Resolvido no documento (05-09-2026), via Claude Web:** em vez de um RF novo, o texto entrou como emenda ao **RF-015** (exclusão de conta), que já existia mas não dizia o que acontecia com o dado pessoal nos registros preservados. `REQUISITOS_V5.md` já traz o texto emendado: "Registros de transações financeiras e logs de auditoria vinculados são preservados pelo prazo legal, **com os dados pessoais do titular substituídos por valores anonimizados, de modo que o registro permaneça íntegro para fins contábeis e de auditoria sem permitir a reidentificação da pessoa**." Bate exatamente com o que o `.sql` já faz. **(2) continua pendente** - é trabalho de front (React), fica pra quando a página pública de campanha/perfil for construída.
...
🔁 **Atualização (05-09-2026): `orcamento_min_itens` mudou de 3 pra 1.** Conferindo `REQUISITOS_V5.md` (RF-039) contra o banco, o texto oficial do requisito citava "valor padrão de 1 (mínimo)" - divergência com o 3 acima. Decisão do Lucas: ajustar o banco pro texto do requisito. `cronograma_min_marcos` não mudou, continua 3 (RF-041 já citava certo). Ver `MATRIZ-RASTREABILIDADE-RF.md`, nota no RF-039.
