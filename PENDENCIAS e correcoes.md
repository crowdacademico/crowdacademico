...
🟢 **746. `arquivo_atualizacao` (dentro de `15-atualizacao-campanha`) já tem endpoint de criação, mas não é testável de ponta a ponta.** O endpoint `POST /arquivo-atualizacao` (vincula um `id_arquivo` já existente a uma atualização de campanha) foi implementado — é um `INSERT` simples na tabela de associação, RLS e grants já prontos desde antes. Só não há como testar de verdade porque `25-arquivo` (o módulo que gera um `id_arquivo` válido de verdade, responsabilidade da Alexia) ainda não existe. O mesmo vai valer pra `arquivo_recompensa` quando `18-recompensa` for implementado.
...
> Sugestão do *** CLAUDE *** (aplicada): usar o padrão de injeção de dependência do próprio NestJS — registrar o `Pool` do `pg` como um provider único do módulo e injetar ele nos services, em vez de cada service criar sua própria instância de `Pool`.
...
*(Nota do Claude Web, confirmada — não é bug, é fronteira certa entre banco e backend: `suspender_pesquisador()` só muda `status` da campanha para `'encerrado_moderacao'`. A devolução de dinheiro de verdade (marcar contribuição como `a_devolver`, chamar a API do gateway) não está automatizada no banco — exige chamar API externa, isso é trabalho do NestJS reagindo à mudança de status, igual já faz pro encerramento normal. Documentado em `DOCUMENTACAO_BD.md [03-P]` pra não deixar passar batido.)*

### parte 13 — segunda rodada do Claude Web sobre o mesmo tema: reativação de pesquisador + RF-056 configurável — **os 2 achados 🟢 corrigidos no mesmo dia, 30-07-2026**

*(O Claude Web conferiu por grep direcionado no `.zip` que as alegações da parte 11/12 batiam com o código de verdade — confirmado. Depois recomendou dois itens novos: implementar a reativação de pesquisador que tinha ficado pendente (item 60), e resolver o RF-056 que também tinha ficado como pergunta em aberto.)*
...
### parte 20 — CRÍTICO: `try/catch` em volta de uma query fazia login não persistir NADA, silenciosamente — achado pelo Claude Code testando o cadastro/sessões de ponta a ponta — **09-08-2026**

*(Contexto: implementando o Bloco B/C do prompt do Claude Web — `listar_papeis_usuario()`, nova função `SECURITY DEFINER` — precisei que o login continuasse funcionando mesmo que essa função ainda não existisse no banco de produção (só existe depois de alguém colar `ATUALIZAR O SUPABASE.sql` no SQL Editor, ver item 22). A "solução" óbvia — `try { ... } catch { return [] }` em volta da chamada — quebrou o login inteiro, de um jeito que NENHUM teste manual via `curl` pegava sozinho: a resposta HTTP vinha 200, com dado que parecia certo.)*
