...
🟢 **748. `18-recompensa` (+ `link_recompensa`/`arquivo_recompensa`) ainda não implementado - parado de propósito.** Na mesma rodada que implementou `12-campanha`, `13-orcamento-campanha`, `14-marco-cronograma`, `15-atualizacao-campanha` (+ satélites) e `16-seguir-campanha`/`17-comentario`, o Lucas pediu explicitamente pra parar em `17-comentario` - `18-recompensa` fica pra uma próxima rodada, não é esquecimento.

🟢 **747. `comentario`: cálculo de `ordem_endosso` feito no Nest, com race condition teórica aceita por ora.** `ComentarioServiceCreate`/`ComentarioServiceUpdate` (`17-comentario`) calculam o próximo `ordem_endosso` livre com um `SELECT MAX(ordem_endosso)+1` antes do `INSERT`/`UPDATE` - a trigger `validar_comentario_endosso` (`05`, `[05-K-3]`) só valida a CONTAGEM contra o limite configurado, não calcula o número. Duas pessoas endossando a mesma campanha ao mesmo tempo podem, em teoria, calcular o mesmo número (não há trava de concorrência). Aceitável por ora dado o volume baixo de uso; se virar problema real, a solução é mover esse cálculo pra dentro de uma trigger `BEFORE INSERT` no banco (mesmo padrão de outras colunas "carimbadas" automaticamente, como `taxa_plataforma`).
...
🟢 **33. Matriz de rastreabilidade RF × banco - FEITA (01-09-2026)**

Uma auditoria de IA identificou esta como "a fala mais importante do WhatsApp inteiro": a Alexia pediu pra "passar as tabelas mais uma vez e ver se cobrem as necessidades, porque vamos ter que iniciar o backend". A Lista C já tinha achados soltos disso (colunas faltando do item 19, taxa não carimbada do item 20), mas faltava a varredura completa nos dois sentidos - pegar os RFs da Etapa 3 e marcar, um por um, se o banco sustenta, sustenta parcialmente ou não sustenta.
...
> Sugestão da *** IA ***: duas coisas, nenhuma delas é mudar o `.sql`. Primeiro, confirmar no próprio Supabase se o papel usado no SQL Editor deles (geralmente `postgres`) tem `BYPASSRLS` - normalmente tem, mas vale confirmar antes de contar com isso, não depois de um deploy dar errado. Segundo, o `tutorial-rodar-projeto.md` merece uma linha explícita dizendo isso: "o `07` (e qualquer re-execução do seed) precisa rodar como superusuário ou papel com `BYPASSRLS` - nunca como `app_nestjs`".

> **Confirmado por completo (04-09-2026):** o Lucas rodou `SELECT rolname, rolsuper, rolbypassrls FROM pg_roles WHERE rolname = current_user;` no SQL Editor do Supabase - resultado: `postgres`, `rolsuper = false`, `rolbypassrls = true`. Ou seja, o papel que o SQL Editor usa não é superusuário, mas tem `BYPASSRLS` marcado separadamente, que é o que basta pra ignorar a RLS - o `07_seed_dados.sql` (e qualquer re-execução) roda ali sem nenhum dos erros de `row-level security policy` que este item alertava. Item fechado.
>
> **Metade confirmada na prática (09-08-2026, teste de IA):** `app_nestjs` (a conexão de `nest/.env`, `DATABASE_URL`) definitivamente NÃO tem `BYPASSRLS` (`rolbypassrls = false`, `rolsuper = false`, conferido direto em `pg_roles`). Isso pegou de um jeito novo, não coberto pelo resto do item: criar uma função `SECURITY DEFINER` inteira via essa conexão (`CREATE FUNCTION` em si NÃO dá erro - `app_nestjs` tem `CREATE` no schema `public`) produz uma função que **parece certa mas não bypassa RLS nenhuma**, porque quem bypassa é o DONO da função, não a cláusula `SECURITY DEFINER` sozinha - e o dono, nesse caso, vira `app_nestjs` (sem privilégio), não `postgres` (que é quem é dono de toda função `SECURITY DEFINER` pré-existente e funciona). O erro só aparece depois, na hora de USAR a função contra uma tabela `FORCE ROW LEVEL SECURITY` (`new row violates row-level security policy`), não na hora de criar - fácil de achar que "funcionou" e descobrir o contrário só mais tarde. **Regra a partir de agora:** qualquer função `SECURITY DEFINER` nova precisa ser criada pelo SQL Editor do Supabase (via `ATUALIZAR O SUPABASE.sql`), nunca direto pela `DATABASE_URL` de `app_nestjs` - mesmo que o `CREATE FUNCTION` "funcione" sem erro. `listar_papeis_usuario`/`registrar_aceite_termo` (09-08-2026) foram criadas errado desse jeito, descobertas quebradas no mesmo dia, e removidas - ficam só no `.sql`, pendentes de alguém colar no SQL Editor de verdade.
...
**Esclarecimento (05-09-2026, achado revisando uma lista de pendências):** tecnicamente semear `recompensa` não depende do módulo `18-recompensa` (Nest) existir - é só `INSERT` direto no seed. Na prática, porém, não faz sentido fazer isso **antes** do módulo: sem tela/endpoint nenhum, as linhas de exemplo não apareceriam em lugar nenhum do sistema rodando, seria só dado morto no banco. Faz mais sentido semear exemplo **junto** de quando `18-recompensa` for construído (`PROXIMOS_MODULOS.md`, Grupo 5), não antes, como um item separado.

### Lista C - travado até vocês dois decidirem (nada mexido aqui)

🟢 **12. O motor de score não tem nenhum requisito escrito - RESOLVIDO no documento (05-09-2026)** *(atualização 28-07-2026: o risco mais sério - exposição pública de LGPD - já foi fechado, ver "🟢 Já corrigido" mais abaixo.)*

**Achado numa segunda auditoria de IA (05-09-2026), revisando uma lista de pendências gerada por uma auditoria anterior:** já está formalizado, não faltava mais nada aqui - a versão anterior estava lendo um texto desatualizado. **RF-030** diz literalmente que a pontuação "não deve, em nenhuma circunstância, bloquear automaticamente a criação de campanhas ou qualquer ação do pesquisador. Serve exclusivamente como sinal de apoio à revisão manual da curadoria" - exatamente "score é sinal interno de curadoria", já escrito, sem precisar de RF novo.
...
🟢 **14. O domínio de recompensa não tem nenhum requisito escrito - RESOLVIDO no documento (05-09-2026)** *(atualização 28-07-2026: parte disso já foi resolvida - ver item 34 no topo do arquivo.)*

**Mesmo achado do item 12, mesma data:** **RF-093** já descreve recompensa simbólica em detalhe - "três tipos: reconhecimento..., acesso antecipado..., ou entrega digital... Recompensas com contrapartida física não são permitidas na plataforma." Já escrito, não precisa de RF novo.
...
>
> **Bônus da mesma conversa - feature "Em breve" (rascunho agendado):** você decidiu, com a Alexia e com apoio de uma ferramenta de IA, que o pesquisador pode escolher lançar a campanha na hora ou agendar um início futuro - mesma ideia do Catarse, com contador regressivo no front. Isso não precisou de status novo nem de coluna nova: a campanha aprovada já fica pública (a policy libera por status), só passou a não poder receber nenhuma doação antes de `data_inicio` chegar (`fn_valida_contribuicao_campanha_ativa`, `05`) - comparado em tempo real, sem precisar de job/cron pra "virar ativa". E as datas (`data_inicio`/`data_fim`) só congelam quando a campanha **de fato** começa (`data_inicio` no passado), não no momento da aprovação - assim o pesquisador pode reagendar o início livremente enquanto ainda está "Em breve". Ver detalhamento completo na seção de resolvidos.

🟢 **17. Campanha de usuário excluído continua pública - RESOLVIDO no documento (05-09-2026)** *(atualização 28-07-2026: testado de verdade, e já funciona melhor do que o diagnóstico original supunha - ver abaixo.)*
...
> Sugestão da *** IA ***: o caminho que resolve os dois lados é anonimizar o autor, não apagar a campanha. A campanha continua existindo com todo o histórico financeiro intacto (atendendo o RNF-007 e protegendo quem doou, que tem direito de ver para onde foi o dinheiro), mas passa a ser exibida como pesquisador removido, sem nome, sem vínculo institucional e sem links. É exatamente o que plataformas de doação fazem quando alguém encerra a conta, porque a alternativa (fazer a campanha sumir) prejudica justamente o doador, que é a parte mais vulnerável da relação. Isso precisa virar um RF novo na Etapa 3 definindo o que é excluído, o que é anonimizado e o que é retido por obrigação legal - hoje o RNF-003 promete a exclusão de forma genérica sem dizer como, e é o tipo de lacuna que uma banca atenta pergunta.

> **Confirmado por teste (28-07-2026):** o `.sql` já faz exatamente isso, sem nenhuma linha de código nova - a correção A9 (27-07-2026) resolveu isso de lambuja. O que faltava não era `.sql`: (1) escrever o RF na Etapa 3 definindo o que é apagado/anonimizado/retido; (2) garantir que o front do NestJS/React exiba algo como "Pesquisador removido" em vez de quebrar quando o `JOIN` com o perfil não retornar nada - isso é trabalho de aplicação, fica pra quando o Nest/React existirem.

> **(1) Resolvido no documento (05-09-2026), via uma IA:** em vez de um RF novo, o texto entrou como emenda ao **RF-015** (exclusão de conta), que já existia mas não dizia o que acontecia com o dado pessoal nos registros preservados. Os Requisitos Funcionais já trazem o texto emendado: "Registros de transações financeiras e logs de auditoria vinculados são preservados pelo prazo legal, **com os dados pessoais do titular substituídos por valores anonimizados, de modo que o registro permaneça íntegro para fins contábeis e de auditoria sem permitir a reidentificação da pessoa**." Bate exatamente com o que o `.sql` já faz. **(2) continua pendente** - é trabalho de front (React), fica pra quando a página pública de campanha/perfil for construída.
...
🟢 **2. Debate `tipo_link` / `contexto_link` - DECIDIDO (05-09-2026)**

Debate sobre criar 2 tabelas novas (`tipo_link` e `contexto_link` reformulados) - era **adiado, não mexer sem pedir**.

> Sugestão da *** IA ***: olhando Catarse e Experiment, os dois têm um conjunto de contextos de link bem pequeno e estável (perfil do criador, página do projeto, atualizações) - não é algo que cresce toda hora na prática. Isso favorece o modelo atual (3 flags booleanas: `permite_perfil`, `permite_atualizacao`, `permite_recompensa`), que é mais simples de ler e de mexer no dia a dia. Eu só migraria pra uma tabela `contexto_link` dinâmica (como o Anexo D do `DOCUMENTACAO_BD.md` já esboça) se vocês já tiverem um plano concreto de adicionar um 4º ou 5º contexto em breve - sem esse plano concreto, a complexidade extra da tabela nova não se paga ainda.

**Achado numa segunda rodada de discussão (05-09-2026), cruzando os Requisitos Funcionais contra o `tipo_link` de verdade:** o texto do requisito (RF-020) ainda listava `Site Institucional`/`Outro`, removidos do catálogo em 15-08-2026 por permitirem link sem verificação de domínio - uma divergência texto-vs-banco não notada até então. Como a remoção foi definitiva (não um "esquecimento" a resolver), isso na verdade **reforça** o argumento de manter as 3 flags: dos 5 tipos ativos, só `GITHUB` usa mais de um contexto, e não há 4º contexto esperado. Requisitos corrigidos pra refletir isso.

**Decisão final:** manter as 3 flags booleanas agora. Caminho de migração pré-decidido, caso um 4º contexto apareça algum dia: vetor de `ENUM` (`contextos contexto_link[]`), não a tabela dinâmica - ver parágrafo completo em `DOCUMENTACAO_BD.md`, Anexo D.
...
> Sugestão da *** IA *** (aplicada): implementar como um interceptor/middleware global do NestJS (não espalhado controller por controller) - assim fica impossível esquecer o `SET LOCAL` numa rota nova que alguém criar depois.

**Implementado exatamente como sugerido, com um refinamento de uma auditoria de IA:** não usei `Scope.REQUEST` do Nest (propagaria escopo request-scoped pra toda a árvore de injeção que toca banco - 27 módulos futuros teriam que lembrar de marcar isso certo). Usei `nestjs-cls` (`AsyncLocalStorage`) - o contexto viaja "por fora", nenhum service precisa saber que existe. `GlobalDbInterceptor` (`nest/src/commons/database/global-db.interceptor.ts`) roda em toda requisição: abre um `Client` do pool, `BEGIN`, `SELECT set_config('app.id_usuario_atual', ...)` parametrizado (nunca string interpolada), cria um Kysely vinculado a ESSE client específico (`kysely-single-connection.dialect.ts` - um Driver/Dialect customizado, transação já gerenciada por fora, então `beginTransaction`/`commitTransaction` do Kysely são no-op de propósito) e guarda no CLS; COMMIT/ROLLBACK + `client.release()` sempre, nos dois casos. `DatabaseService.getDb()` é o único ponto que qualquer service usa.

**Teste de integração de verdade - RODADO E CONFIRMADO (01-08-2026, em auditoria de IA com banco disponível).** Numa sessão de IA sem Postgres/Docker no próprio ambiente, só foi possível compilar/lintar; outra sessão, com Node+Postgres disponíveis, rodou o backend de verdade: subiu o servidor contra o Postgres real, logou como 2 usuários reais (admin e pesquisador comum) e disparou 15 requisições concorrentes intercaladas de cada um contra `PATCH /configuracoes/5` - 15/15 do admin voltaram `200`, 15/15 do pesquisador voltaram `403`, **zero contaminação cruzada** (nenhuma pegou a conexão/sessão do outro). Confirmou também, lendo o código, que o `403` vem da RLS de verdade recusando o `UPDATE` (Postgres bloqueando 0 linhas, o service só diferencia depois "não existe" de "RLS bloqueou" com um `SELECT`), não de um guard de aplicação decorativo. Sem token ou com token forjado: `401` nos dois casos. Esse era o único ponto em aberto desde que a Pendência 5 foi desenhada - está fechado.
...
> Sugestão da *** IA *** (aplicada): um health-check simples na subida do NestJS (`SELECT current_user`) que impede a aplicação de subir se a conexão não for exatamente `app_nestjs` - evita o erro silencioso de rodar com superusuário sem ninguém perceber, o que faria a RLS inteira parecer que "funciona" em teste mas não proteger nada de verdade.
...
Era só nota de melhoria futura, não urgente - mas acabou resolvida sem ter sido pedida diretamente, como consequência da fundação construída pra Pendência 5 (contexto de sessão por requisição).

> Sugestão da *** IA *** (aplicada): usar o padrão de injeção de dependência do próprio NestJS - registrar o `Pool` do `pg` como um provider único do módulo e injetar ele nos services, em vez de cada service criar sua própria instância de `Pool`.

**Confirmado nesta revisão (verificado direto no código, não só de memória):** `nest/src/commons/database/database.module.ts` é o único lugar do projeto que faz `new Pool(...)` como provider do módulo (fora de `aplicar-migrations.script.ts`, que é um script avulso, não um service). Todo service acessa o banco só via `DatabaseService.getDb()` - nenhum service abre `pool.query()`/`pool.connect()` próprio. Ver detalhamento completo na "parte 16", mais abaixo (seção Nest/React).
...
### parte 2 - uma auditoria de IA rodou de verdade num Postgres e achou um bug crítico - **todos 🟢 corrigidos no mesmo dia**
...
*(Você bateu o martelo direto com a Alexia - sem precisar de mais rodada de revisão - no prazo de campanha (60 dias, não 90) e trouxe uma feature nova: campanha pode ser lançada na hora ou agendada, tipo "Em breve" da Catarse. De quebra, resolveu 13(c)/13(d)/15 com respostas curtas dela no WhatsApp, e apareceu um bug real que a IA dela achou.)*
...
🟢 **BUG - `campanha.encerrado_em` nunca era preenchido - encontrado pela IA usada pela Alexia, CORRIGIDO**
...
*(Você aplicou direto a sugestão de uma IA no item 3, confirmou "sim, mantém" pro item 19(a)/(c), e pediu pra aplicar minha própria sugestão de 7 tipos no `tipo_link` - não havia sugestão de outra ferramenta de IA registrada sobre esse ponto específico.)*

🟢 **3 (fechamento). `score_minimo_campanha` vira sinal de revisão manual, sem trigger de bloqueio**

`public.fn_precisa_revisao_score(p_id_usuario)` nova em `05` (`[05-I-1]`, `STABLE SECURITY DEFINER`) - retorna `TRUE` se o score do pesquisador estiver abaixo de `configuracoes.score_minimo_campanha`. Nenhuma trigger bloqueia `INSERT` em `campanha` com base nisso - de propósito, seguindo a sugestão de uma IA de que o filtro de confiança real é a aprovação manual do Admin, não um score acumulado. O comentário `TODO` do seed foi trocado por um comentário explicando a decisão e apontando pra função.
...
- 5 chaves novas em `configuracoes`, todas editáveis pelo Painel Admin: `limite_caracteres_descricao_campanha` (5000), `limite_caracteres_conteudo_atualizacao` (5000), `limite_caracteres_relato_denuncia` (1000 - sugestão da própria auditoria de IA), `limite_caracteres_justificativa_encerramento` (2000, compartilhada pelas 2 colunas de justificativa), `limite_caracteres_descricao_recompensa` (2000).

🟢 **45. Quatro alavancas fantasma em `configuracoes` - CORRIGIDO**

Varredura inversa feita em auditoria de IA: pra cada chave em `configuracoes`, quantas vezes ela é lida em algum dos 8 arquivos. 4 não tinham nenhum consumidor - o Admin muda no painel e nada acontece, pior que um valor fixo no código, porque parece que devia funcionar.
...
**Prova mecânica desta parte 6:** 39 tabelas (igual). `PK_`=39 (igual), `FK_`=56 (igual), `UK_`=19 (igual), `CK_`=21 (+6: as 6 `CK_..._TAMANHO`). Parênteses balanceados em `01`, `03`, `05`, `06` e `07` (saldo zero nos cinco - o desbalanceamento de 1 parêntese encontrado no meio da edição, num comentário, foi pego e corrigido antes de fechar). Funções de `03`: 10 (+5: as funções de autenticação, `[03-O]`). Funções de `05`: 37 (+1: `fn_valida_limite_texto_livre`). Triggers de `05`: 40 (+6: os 6 triggers de limite de texto). Policies: continuam 105 (nada mudou em `04` nesta parte - a correção foi 100% em `GRANT`, não em RLS). `configuracoes`: 4 chaves novas (login) + 5 chaves novas (limite de texto) − 2 removidas (fantasma) = líquido +7 linhas. Testado em auditoria de IA, os 4 ataques do achado 44 (forjar score, auto-verificar e-mail, limpar bloqueio, ressuscitar conta): todos `permission denied` depois da correção; editar título/vínculo em `perfil_pesquisador` e `recalcular_score_pesquisador()`/`recalcular_todos_os_scores()` continuam funcionando normalmente (são `SECURITY DEFINER`). Simulação de score pros 4 pesquisadores do teste de faixas: Bruno 100, Renata 60, Eduardo 48, Vinícius 19 - inalterado (nada nesta parte toca as 4 dimensões do cálculo).

### parte 7 - 3ª rodada de auditoria de IA: SECURITY DEFINER sem checar quem chama, cascata de score nunca contida, 16 FKs sem índice - **os 7 itens da lista dele 🟢 corrigidos no mesmo dia**

*(Uma auditoria de IA rodou tudo do zero de novo, confirmou os 4 furos da parte 6 fechados, e foi um passo além: testou as 5 funções novas COMO um atacante testaria - chamando com o id de outra pessoa. Achou que a correção da parte 6 trocou um furo por um pior. Também mediu gravações de banco em vez de só ler código, e achou uma cascata de recálculo de score que ninguém tinha pego em 10 rodadas anteriores.)*

🟢 **46. As 5 funções SECURITY DEFINER de `[03-O]` não checavam quem estava chamando - FURO MAIOR QUE O ORIGINAL, CORRIGIDO**

`SECURITY DEFINER` desliga a RLS - a função vira a ÚNICA guardiã. A 1ª versão (parte 6) aceitava qualquer `p_id_usuario` sem checagem nenhuma. Testado em auditoria de IA como usuário comum (id 9): `confirmar_email_usuario(2)`, `liberar_bloqueio_login(2)` e `excluir_conta_usuario(2)` - todas executavam contra a conta do pesquisador 2. Um usuário comum conseguia excluir a conta de **qualquer outra pessoa**. Pior que o `GRANT UPDATE` de tabela inteira que essas funções vieram substituir (lá pelo menos `pol_usuario_update` restringia a `id_usuario_atual() = id_usuario`).

**Três correções diferentes, testadas em auditoria de IA antes de eu aplicar:**
...
🟢 **47. Cascata de recálculo de score nunca contida - item aberto desde a 1ª auditoria de IA, CORRIGIDO**

`trg_campanha_recalcula_score` era `AFTER INSERT OR UPDATE OR DELETE` sem `WHEN` - todo `UPDATE` em `campanha` recalculava as 4 dimensões inteiras do score do dono, mesmo quando nada relevante mudou. A cadeia `contribuicao` → `trg_sincroniza_arrecadado_campanha` → `UPDATE campanha` (só `valor_bruto_arrecadado`) → esta trigger disparava um recálculo completo **por doação confirmada**. Medido em auditoria de IA instrumentando as gravações: 5 doações = 20 gravações em `score_pesquisador` (4 por doação) - todas com o mesmo resultado, porque `valor_bruto_arrecadado` não entra em nenhuma das 4 dimensões. Numa campanha com 500 doações seriam 500 recálculos completos, cada um segurando o `FOR UPDATE` da linha da campanha - risco direto pro RNF-006 (confirmação de pagamento em até 30s).
...
Postgres, diferente de outros bancos, não cria índice automático em coluna de FK. Das 56 FKs do banco, 16 estavam sem. A auditoria de IA priorizou 5, por impacto real de produto:
...
🟢 **51. Colisão de letra pré-existente: `[03-F]`/`[03-G]` reaproveitavam letras que já pertenciam a outros domínios - CORRIGIDO**

Achado em 09-08-2026 escrevendo o Bloco G (moderação/suspensão) - mesma classe de bug já corrigida uma vez neste projeto (`[03-K]`→`[03-M]`), só que mais antiga. O Índice Global de Letras (topo de `DOCUMENTACAO_BD.md`) atribui `F` a LINK (Vinculação de URLs Externas) e `G` a ARQUIVO (Vinculação de Mídias) - mas `03_funcoes_seguranca.sql` tinha `[03-F]` ("OPERAÇÕES DE AUTENTICAÇÃO": `confirmar_email_por_token`, `registrar_falha_login`, `liberar_bloqueio_login`, `registrar_login_sucesso`, `excluir_conta_usuario`) e `[03-G]` ("MODERAÇÃO SOBRE PESQUISADOR": `suspender_pesquisador`, `reativar_pesquisador`), nenhum dos dois relacionado a LINK/ARQUIVO. Registrado como pendência na mesma rodada em que foi achado (por isso o bloco novo daquela rodada, `[03-N]`, moderação de conta, escolheu uma letra livre em vez de piorar a colisão).

**Corrigido (09-08-2026, mesmo dia, pedido do Lucas: "é só renomear rótulo em comentário, não mexe em schema"):** `[03-F]`→`[03-O]` e `[03-G]`→`[03-P]`, letras livres - renomeado em todos os comentários que apontavam pra eles (`01`, `03`, `05`, `06`, `07`, `08`, `DOCUMENTACAO_BD.md`, NestJS, React), Índice Global de Letras atualizado com as duas novas linhas. 100% comentário/documentação - nenhuma DDL, função, policy ou grant mudou; não precisa do SQL Editor do Supabase.

🟢 **50. Três funções privilegiadas que escrevem ainda estavam com `EXECUTE` liberado pra `PUBLIC` - CORRIGIDO**

A rodada anterior (parte 7) fez `REVOKE EXECUTE ... FROM PUBLIC` nas 5 funções de `[03-O]`, mas ficaram de fora 3 que também escrevem no banco: `atribuir_papel_padrao(INT)` (`08`, escreve em `usuario_papel`), `recalcular_score_pesquisador(INT)` e `recalcular_todos_os_scores()` (`05`, escrevem em `score_pesquisador`/`perfil_pesquisador`). A última é a mais séria das três mesmo com risco "baixo" - sem custo de chamada nenhum pra quem chama, é negação de serviço barata deixada aberta (percorre todos os pesquisadores a cada chamada, bastaria um laço). Hoje não é explorável (só `app_nestjs` conecta ao banco), mas a auditoria de IA notou certo: já que o `REVOKE` foi feito pras 5, deixar essas 3 de fora só cria a pergunta "por que essas sim e essas não" daqui a uns meses. **Corrigido:** `REVOKE EXECUTE ... FROM PUBLIC` + `GRANT` só pra `app_nestjs` nas 3, mesmo padrão das 5 anteriores. As outras 11 funções `SECURITY DEFINER` que continuam em `PUBLIC` são só leitura (`tem_permissao`, `usuario_visivel`, `config_numero`, os 4 calculadores de dimensão de score, os contadores de seguidores, `fn_precisa_revisao_score`) - sem risco de escrita, ficam como estão.

**Errata da parte 7:** a prova mecânica anterior reportou `permissao` com 29 linhas; o número certo, confirmado em auditoria de IA rodando o banco de verdade, é **31** (meu script de verificação tinha um bug de âncora que cortava a contagem cedo demais - corrigido no comando usado nesta parte). Não houve nenhuma linha perdida ou duplicada, foi só erro de relatório, não de dado.
...
### parte 10 - 6ª auditoria de IA: as 5 correções da parte 9 confirmadas + o encerramento automático de campanha não tinha caminho - **o único achado 🟢 corrigido no mesmo dia, + 2 registros de documentação**

*(Uma auditoria de IA rodou tudo de novo, confirmou as 5 correções da parte 9 uma por uma - inclusive testando os 4 ramos da `trg_campanha_valida_transicao` isoladamente como superusuário - e foi um passo além: simulou o cron do RF-037 (encerramento automático de campanha vencida) rodando de verdade, sem sessão de usuário, como um job de fundo roda na prática.)*

🟢 **58. Encerramento automático de campanha vencida (RF-037) não tinha caminho nenhum - CORRIGIDO**

Reproduzido: um job rodando como `app_nestjs` sem `app.id_usuario_atual` definido (exatamente como um cron roda) executava `UPDATE campanha SET status='sucesso' WHERE <vencida, meta batida>` e `UPDATE campanha SET status='nao_atingido' WHERE <vencida, meta não batida>` - as duas devolviam `UPDATE 0`, **sem erro nenhum**. Causa: não é a `trg_campanha_valida_transicao` (item 51, parte 9) - a auditoria de IA testou o ramo autoverificável dela isoladamente, rodando como superusuário, nos 4 casos possíveis (prazo vencido + meta batida → `sucesso` ✅; prazo vencido + meta não batida → `nao_atingido` ✅; tentando mentir, sucesso sem meta → bloqueado ✅; admin com sessão → passa ✅) - a trigger está certa. A causa é `pol_campanha_update` (04): exige ser dono OU ter `campanha_editar`/`campanha_aprovar`/`campanha_rejeitar` - um job não é nenhum dos dois, então a RLS filtra **todas** as linhas antes mesmo da trigger avaliar qualquer coisa. Modo de falha mais perigoso que existe: silencioso - um cron reportaria "ok" toda madrugada sem encerrar nenhuma campanha vencida, que ficaria `'ativo'` pra sempre (página pública com contador regressivo negativo, recusando doações novas com "prazo já encerrado" - o doador veria uma campanha aberta rejeitando o próprio dinheiro).
...
**Prova mecânica desta parte 12:** 39 tabelas (igual, nenhuma nova). `PK_`=39, `FK_`=57, `UK_`=19, `CK_`=22 - todos iguais. Parênteses balanceados em `03`, `05` e `06` (saldo zero, conferido nos três). Policies: continuam 105 (nenhuma nova; `pol_campanha_update`/`pol_perfil_update` não foram tocadas - a cascata passa por cima da RLS via `SECURITY DEFINER`, não por mudar policy). Índices: continuam 42. Permissões: continuam 31 (`usuario_suspender` já existia - passou a ter uso, não é permissão nova). `configuracoes`: continuam 32 linhas. Funções de `03`: +1 (`suspender_pesquisador`). Funções de `05`: continuam 43 (só o corpo de `fn_valida_transicao_campanha` mudou, função já existia). Triggers: sem mudança (46). Teste mental dos 2 ramos: pesquisador com 1 campanha `ativo` (meta não batida, prazo não vencido) e 1 `aguardando_aprovacao` → depois de `suspender_pesquisador(id)`, a `ativo` vira `encerrado_moderacao` e a `aguardando_aprovacao` vira `rejeitado`, sem exigir nenhuma permissão de campanha de quem chamou a função - só `usuario_suspender`. Chamar a função de novo no mesmo pesquisador já suspenso devolve `FALSE` sem tocar em nada (idempotente, não fecha campanhas que já foram reabertas manualmente por engano). Item **60** (reativação) resolvido no mesmo dia - ver `parte 13`, logo abaixo.

*(Nota de uma auditoria de IA, confirmada - não é bug, é fronteira certa entre banco e backend: `suspender_pesquisador()` só muda `status` da campanha para `'encerrado_moderacao'`. A devolução de dinheiro de verdade (marcar contribuição como `a_devolver`, chamar a API do gateway) não está automatizada no banco - exige chamar API externa, isso é trabalho do NestJS reagindo à mudança de status, igual já faz pro encerramento normal. Documentado em `DOCUMENTACAO_BD.md [03-P]` pra não deixar passar batido.)*

### parte 13 - segunda rodada de auditoria de IA sobre o mesmo tema: reativação de pesquisador + RF-056 configurável - **os 2 achados 🟢 corrigidos no mesmo dia, 30-07-2026**

*(Uma auditoria de IA conferiu por grep direcionado no `.zip` que as alegações da parte 11/12 batiam com o código de verdade - confirmado. Depois recomendou dois itens novos: implementar a reativação de pesquisador que tinha ficado pendente (item 60), e resolver o RF-056 que também tinha ficado como pergunta em aberto.)*

🟢 **60. `reativar_pesquisador()` - CONSTRUÍDA**

`reativar_pesquisador(p_id_usuario INT) RETURNS BOOLEAN` (`03`, `SECURITY DEFINER`, `[03-P]`) - mesma permissão `usuario_suspender` de `suspender_pesquisador()` (quem pode suspender pode reverter), devolve `status_pesquisador = 'ativo'`, idempotente (`FALSE` se já estava ativo). **De propósito, não toca em nenhuma linha de `campanha`** - campanha fechada por `encerrado_moderacao` na suspensão continua fechada pra sempre, mesmo depois de reativado. Motivo é financeiro: quando a suspensão rodou, o dinheiro já começou a se mexer (devolução ou repasse, fora do banco); reabrir a campanha depois seria prometer algo que a plataforma não cumpre. Reativação só devolve a capacidade de criar campanha NOVA.

🟢 **RF-056. Valor mínimo de contribuição virou configurável - CORRIGIDO**

`CK_CONTRIBUICAO_VALOR_MINIMO` (`01`) era `CHECK (valor >= 5.00)` hardcoded - confirmado em auditoria de IA que não é piso do gateway de pagamento (o PIX em si não impõe mínimo), é política de negócio da plataforma, mesma família dos outros 4 limites já configuráveis (item 16 da Lista C). **Corrigido, mesmo padrão exato:** a `CHECK` virou só limite técnico largo (`valor > 0`); nova chave `configuracoes.valor_minimo_contribuicao` (5.00, mesmo valor de antes); trigger nova `fn_valida_contribuicao_valor_minimo()`/`trg_contribuicao_valida_valor_minimo` (`05`, `[05-K-2]`, `BEFORE INSERT`) aplica o mínimo de negócio de verdade. Nenhum valor mudou - só o lugar de onde vem, igual todos os outros 4 desta mesma família.

**Prova mecânica desta parte 13:** 39 tabelas (igual). `PK_`=39, `FK_`=57, `UK_`=19, `CK_`=22 - todos iguais (constraint de contribuição foi alterada, não criada nem removida). Parênteses balanceados em `01`, `03`, `05`, `06` e `07` (saldo zero, conferido nos cinco). Policies: continuam 105. Índices: continuam 42. Permissões: continuam 31. Funções de `03`: +1 (`reativar_pesquisador`, total 13). Funções de `05`: **44** (+1: `fn_valida_contribuicao_valor_minimo`). Triggers de `05`: **47** (+1: `trg_contribuicao_valida_valor_minimo`). Teste mental: `INSERT INTO contribuicao (..., valor) VALUES (..., 3.00)` → bloqueado pela trigger nova (abaixo do mínimo configurado); `valor = 5.00` → passa; pesquisador suspenso e depois reativado com uma campanha que tinha sido fechada em `encerrado_moderacao` → continua `encerrado_moderacao` depois da reativação, só uma campanha nova criada depois consegue nascer normalmente.

> 🗑️➡️✅ **Errata (01-08-2026):** a linha acima tinha citado "`configuracoes`: 33 linhas" - contagem errada, minha, não da Alexia. Recontei por dois métodos independentes (grep pelas linhas que começam com `(NULL,` e grep pelas que terminam em `TRUE),`, ambos bateram) e o valor real da época era bem menor. Não reconstruí retroativamente onde exatamente cada rodada anterior errou a conta - só registro que o número certo, medido agora, está na prova mecânica da parte 14, logo abaixo. Mesma prática de correção transparente já usada quando uma auditoria de IA achou a contagem errada de `permissao` (rodada de 28-07-2026).

### parte 14 - Alexia mandou 2 tabelas novas (orçamento e cronograma estruturados) + bug de calibração corrigido antes do commit - **01-08-2026**

*(A Alexia mexeu direto no banco pela primeira vez nesta sessão - sem passar por nenhuma auditoria de IA, direto comigo. Mandou os 6 arquivos alterados pelo WhatsApp pro Lucas revisar antes de subir. Pedi pra examinar com riqueza de detalhes e dar um veredito: achei a implementação tecnicamente muito boa - seguiu à risca os padrões já estabelecidos (RLS forçada, GRANT por tabela, limite configurável via `configuracoes` em vez de hardcoded, reaproveitamento de `fn_valida_limite_texto_livre` em vez de duplicar função, `DROP TRIGGER IF EXISTS` pra idempotência) - mas sinalizei um problema de calibração, não de código: `orcamento_min_itens=10`/`cronograma_min_marcos=20` como MÍNIMO obrigatório pra aprovar qualquer campanha é um número absurdamente alto pra esse tipo de exigência. A própria Alexia confirmou a confusão: os números 10/20 eram pra ser o TETO, não o piso - o mínimo de verdade devia ser bem menor, ela sugeriu entre 2 e 5.)*

🟢 **`orcamento_campanha` e `marco_cronograma` - tabelas novas, CONSTRUÍDAS (Alexia)**

Orçamento estruturado (itens de gasto com categoria + valor, pra gráfico de pizza) e cronograma estruturado (marcos com título/descrição/data prevista), inspirados no Experiment.com. Decisão de produto confirmada pelo Lucas no WhatsApp antes de a Alexia começar: **os dois são obrigatórios**, não opcionais, pra aprovar uma campanha. Implementação: 2 tabelas (`01`, `[01-E]`, 39→41 tabelas), 2 índices de FK (`02`), RLS completo select/insert/update/delete (`04`, `[04-E]` - leitura segue a mesma visibilidade de `pol_campanha_select`, escrita é dono ou `campanha_editar`), GRANT com `DELETE` incluso (`06` - o pesquisador precisa poder remover um item antes de reenviar a campanha), 2 novas chaves de limite de texto (`07`) reaproveitando `fn_valida_limite_texto_livre` (`05`, `[05-K-1]`). Regras de negócio novas em `05`, `[05-K-2]`: orçamento congela na aprovação sem exceção (`fn_congela_orcamento_campanha`, mesma dureza de `fn_congela_regras_campanha`); cronograma só congela quando a campanha começa de fato (`fn_congela_marco_cronograma`, mesma janela de carência do "Em breve"); data de marco não pode ser anterior a `data_inicio` (`fn_valida_data_marco_cronograma`); completude checada na aprovação - mínimo de itens/marcos e soma do orçamento batendo EXATAMENTE com `meta_financeira` (`fn_valida_completude_campanha_aprovacao`).

🟢 **Bug de calibração min/max - CORRIGIDO antes do commit, nunca rodou contra banco real**

A 1ª versão guardava só `orcamento_min_itens`/`cronograma_min_marcos` (10/20) e usava esse valor como PISO obrigatório pra aprovação - nenhuma plataforma de referência (nem o próprio Experiment.com, citado como inspiração) exige 10 categorias de orçamento e 20 marcos de cronograma pra uma campanha pequena; na prática travaria a aprovação de quase toda campanha simples. **Corrigido:** separado em 2 chaves por tabela - `orcamento_min_itens`/`cronograma_min_marcos` (3, o piso de verdade, checado na aprovação) e `orcamento_max_itens`/`cronograma_max_marcos` (10/20, os valores originais da Alexia, agora usados como TETO de verdade). O teto ganhou checagem própria, nova, no momento do `INSERT` (`fn_valida_limite_max_orcamento_campanha`/`fn_valida_limite_max_marco_cronograma`, `05`) - feedback imediato pro pesquisador no item 11/marco 21, em vez de só descobrir o excesso quando a campanha for recusada na moderação. Valor do mínimo (3) é meio-termo da faixa que a própria Alexia sugeriu (2 a 5) - ajustável livremente depois via `UPDATE` em `configuracoes`, sem migração.

🔁 **Atualização (05-09-2026): `orcamento_min_itens` mudou de 3 pra 1.** Conferindo os Requisitos Funcionais (RF-039) contra o banco, o texto oficial do requisito citava "valor padrão de 1 (mínimo)" - divergência com o 3 acima. Decisão do Lucas: ajustar o banco pro texto do requisito. `cronograma_min_marcos` não mudou, continua 3 (RF-041 já citava certo). Ver `MATRIZ-RASTREABILIDADE-RF.md`, nota no RF-039.

**Registro (não é bug, é lacuna de demo):** as 10 campanhas do seed não ganharam linhas de exemplo em nenhuma das 2 tabelas novas - carregam sem erro (a trigger de completude só roda em `UPDATE` pra `'ativo'`, não no `INSERT` do seed), mas qualquer print/demo das telas novas usando o banco de teste vai aparecer vazio pra todas as campanhas antigas.

🟢 **Faltava `SECURITY DEFINER` nas 6 funções novas - achado numa segunda revisão, 01-08-2026, antes do commit**

Não foi pedido - reli o próprio trabalho depois de fechar a parte 14 e achei: as 6 funções novas (`fn_congela_orcamento_campanha`, `fn_valida_limite_max_orcamento_campanha`, `fn_congela_marco_cronograma`, `fn_valida_data_marco_cronograma`, `fn_valida_limite_max_marco_cronograma`, `fn_valida_completude_campanha_aprovacao`) fazem `SELECT`/`COUNT`/`SUM` contra `campanha`/`orcamento_campanha`/`marco_cronograma` sem `SECURITY DEFINER` - ficando sujeitas à RLS de quem está executando. `pol_campanha_select`/`pol_orcamento_campanha_select`/`pol_marco_cronograma_select` (`04`) não incluem `'campanha_editar'` entre as condições de visibilidade, só status/dono/`'relatorio_visualizar'`. Pro dono da campanha nunca foi problema (a condição de dono já libera direto); o risco é: **hoje só `admin` tem `campanha_aprovar`/`campanha_editar`, e `admin` também tem `relatorio_visualizar`**, o que mascarou o bug por acidente - nunca apareceria testando manualmente como admin. Mas é exatamente o tipo de furo que o item 57 (ainda em aberto, ver acima) abriria: se `moderador` ganhar `campanha_aprovar` sem ganhar `relatorio_visualizar` junto, a contagem de itens passaria a ver sempre `0` linhas - bloqueando toda aprovação de campanha pra esse papel, silenciosamente, sem erro nenhum. Mais grave no caso de `fn_valida_completude_campanha_aprovacao`: ela roda em TODA aprovação de campanha, garantido, não é um caminho raro. **Corrigido:** as 6 ganharam `SECURITY DEFINER SET search_path = public` - mesmo padrão já usado em `contar_seguidores_pesquisador()` (`03`) e `encerrar_campanhas_vencidas()` (`05`, item 58, parte 10) pro mesmo motivo: verificação agregada precisa ver o total real do banco, não só o que a sessão de quem chama consegue enxergar linha a linha.

**Prova mecânica desta parte 14:** 41 tabelas (+2). `PK_`=41 (+2), `FK_`=59 (+2), `UK_`=19 (igual, nenhuma unicidade nova), `CK_`=25 (+3: `CK_ORCAMENTO_CAMPANHA_VALOR_POSITIVO`, `CK_ORCAMENTO_CAMPANHA_DESCRICAO_TAMANHO`, `CK_MARCO_CRONOGRAMA_DESCRICAO_TAMANHO`) - todos conferidos por grep direto, não de cabeça. Parênteses balanceados nos 8 arquivos (saldo zero, conferido programaticamente). Policies: **113** (+8: select/insert/update/delete × 2 tabelas). Índices: **44** (+2). Permissões: continuam 31 (nenhuma nova - reaproveitou `campanha_editar`/`relatorio_visualizar`). `configuracoes`: **27 linhas** (contagem real, ver errata acima) - deste total, 6 são desta rodada (`orcamento_min_itens`, `orcamento_max_itens`, `cronograma_min_marcos`, `cronograma_max_marcos`, `limite_caracteres_descricao_orcamento`, `limite_caracteres_descricao_marco`). Funções de `05`: **50** (+6). Triggers de `05`: **55** (+8). Funções de `03`: continuam 13 (nenhuma nova). Teste mental: campanha com 2 itens de orçamento tenta ser aprovada → bloqueada (`< orcamento_min_itens=3`); com 3 itens somando exatamente a meta e 3 marcos → aprovação passa; tentativa de inserir o 11º item de orçamento → bloqueada por `fn_valida_limite_max_orcamento_campanha` antes mesmo de chegar na aprovação.

### parte 15 - bug de trava circular em `fn_congela_marco_cronograma`, achado numa auditoria de IA rodando o banco de verdade - **01-08-2026**

*(Lucas mandou o `.zip` do projeto para uma ferramenta de IA auditar código + os 3 `.docx` de requisitos depois da reorganização de módulos. A auditoria não leu de cabeça - rodou o bootstrap `01`→`08` contra um banco real e reproduziu o bug ao vivo antes de reportar.)*

🟢 **Bug real - `fn_congela_marco_cronograma` travava o cronograma ANTES da campanha ser aprovada - CORRIGIDO**

A versão da parte 14 (`fn_congela_marco_cronograma`, `05`, `[05-K-2]`) só checava `campanha.data_inicio <= NOW()` pra decidir se o cronograma já devia estar congelado - diferente das suas duas funções irmãs (`fn_congela_regras_campanha`, `fn_congela_orcamento_campanha`), que só travam com a campanha **já aprovada** (`status IN ('ativo', ...)`). Cenário reproduzido em auditoria de IA: pesquisador cria a campanha com `data_inicio = NOW()` (não usa o agendamento "Em breve"), a campanha fica em `aguardando_aprovacao` esperando moderação; assim que o relógio passa de `data_inicio` - poucos segundos depois, moderação normal - a trigger já bloqueia qualquer `INSERT` em `marco_cronograma`. Como `fn_valida_completude_campanha_aprovacao` exige no mínimo `cronograma_min_marcos` (3) marcos pra deixar aprovar, e a campanha não tinha nenhum ainda, isso trava em círculo: não aprova sem marco, não consegue criar marco porque "já começou" - sem a campanha nunca ter sido de fato aprovada. O orçamento (`fn_congela_orcamento_campanha`) não tinha esse problema, porque já usava `status` desde a parte 14. **Corrigido:** acrescentada a mesma condição de `status` das funções irmãs - `fn_congela_marco_cronograma` agora só bloqueia se `status IN ('ativo','sucesso','nao_atingido','encerrado','encerrado_moderacao')` **E** `data_inicio <= NOW()`, preservando a janela de carência do "Em breve" (cronograma seguirá editável entre a aprovação e o início de fato) sem travar campanhas ainda em moderação.

*(Na mesma auditoria de IA, foi conferido - sem achar problema - que os 3 `.docx` de requisitos (Etapa 1/2/3) já batem com o código atual: RS-05 em PostgreSQL, RNF-015 e RF-024 sobre score automatizado presentes, RF-032 a RF-036 cobrindo orçamento/cronograma exatamente como implementado, e que "MySQL" não aparece mais em nenhum dos dois documentos de etapa anterior. Esse lado é trabalho feito com apoio de ferramentas de IA no `.docx` - não mexi nele.)*
...
> Sugestão da *** IA ***: `SELECT NULLIF(current_setting('app.id_usuario_atual', true), '')::INT;` - uma palavra (`NULLIF`) resolve os três casos (não definida, definida vazia, definida com valor) corretamente.
...
> Sugestão da *** IA ***: separar a trigger em duas - uma `BEFORE INSERT` sem condição (comportamento original) e outra `BEFORE UPDATE` com uma cláusula `WHEN (NEW.meio_pagamento IS DISTINCT FROM OLD.meio_pagamento OR NEW.id_campanha IS DISTINCT FROM OLD.id_campanha)`, só revalidando quando o que importa de fato muda. Isso mantém a proteção que a A3 queria (impedir trocar o meio de pagamento por baixo dos panos) sem travar o fluxo normal de confirmação.
...
> Sugestão da *** IA ***: manter `data_fim` congelado, adicionar `data_inicio` na mesma lista de campos protegidos do B2 (mesma trigger, mesmo padrão), e criar uma coluna nova `encerrado_em TIMESTAMP` em `campanha`, preenchida em qualquer tipo de encerramento. Isso também serve ao RF-058 (data do repasse no painel) e à auditoria em geral.
...
# AVALIAÇÃO DE FERRAMENTAS DE IA SOBRE O SISTEMA:
...
O próprio Anexo A do `DOCUMENTACAO_BD.md` (que consolidou o antigo `PLANO_AUTENTICACAO_PROPRIA.md`, já removido do repositório) já avisava isso na época: "Estritamente, [manter RLS] não é necessário", já que o NestJS é o único client do banco - a recomendação foi manter mesmo assim, como camada extra. Isso significa que toda regra de autorização precisa ser escrita duas vezes: uma vez em policy SQL, outra vez em guard/service do NestJS (o próprio Anexo B do `DOCUMENTACAO_BD.md`, que consolidou o antigo `RBAC-pontos-discutidos.md`, já previa isso). Hoje isso já é ~105 policies + 24 triggers só de regra - pra duas pessoas mantendo sozinhas, esse é o tipo de coisa que droga com o tempo (uma trava muda na aplicação e ninguém lembra de espelhar no banco, ou vice-versa). Não é errado ter feito assim, é uma escolha de segurança válida - só que é bom vocês saberem que estão pagando esse preço conscientemente, não de graça.
...
*(Lucas pediu para uma ferramenta de IA opinar sobre a stack (Node/Nest/React/Postgres) - ela validou e destacou dois pontos técnicos ("Probleminha-chan.md"): como resolver SET LOCAL + pool de conexão (Pendência 5) sem `Scope.REQUEST`, e Kysely+kysely-codegen como ORM. Lucas pediu pra implementar os módulos que a própria auditoria de IA recomendou como primeiro entregável: `1-usuario` (já existia), `3-auth`, `2-papel-permissao`, `11-configuracoes` - e uma tela mínima de devtools no React, só pra ver CRUD + RLS funcionando, sem construir o site ainda.)*
...
**O que não foi possível testar nesta rodada (numa sessão de IA sem banco disponível):** este ambiente (sandbox de IA) não tem Postgres nem Docker disponível - só o código. `npm run build`/`npm run lint`/`npm run test` passam limpos nos dois projetos (nest/ e react/), mas o comportamento em tempo de execução ficou pendente.

**Atualização no mesmo dia - outra sessão de IA, com Postgres disponível, rodou o teste real:** backend subiu de primeira contra o Postgres de verdade, health-check (`app_nestjs`) passou, login funcionou com 2 usuários reais, e o teste de concorrência (15 requisições simultâneas de cada usuário contra a mesma rota protegida) confirmou isolamento perfeito - ver item 5 na seção do banco (acima da linha de separação Nest/React) pro detalhe completo. Único imprevisto: um erro da própria auditoria de IA tentando trocar senha via `psql -c` com hash bcrypt interpolado direto no shell (o `$` do hash foi engolido como variável de posição) - não é bug do projeto, só ruído de shell, resolvido escrevendo o `UPDATE` num `.sql` em vez de interpolar na linha de comando.
...
*(Sequência de pedidos do Lucas, todos no mesmo dia, cada um construindo em cima do anterior: painel sem portão de login → identidade visual do Projeto de Interface real → menu lateral colado nas bordas → tela de criar usuário com toast → uma auditoria de IA revisou tudo e achou uma pergunta de arquitetura real, que o Lucas decidiu resolver na hora.)*
...
🟢 **Auditoria de IA + pergunta de arquitetura: abas viravam URL ou ficavam como estado?**

Pedi para uma ferramenta de IA revisar a rodada de CSS/UX (skeleton de carregamento, "Nenhum registro" vs. erro, drawer mobile, rename devtools→crud, build/lint) - confirmou as 5 alegações rodando o código, não só lendo. Achado dela: eu tinha dito "uma lista só, App.jsx e breadcrumb.jsx leem dela" mas **não** incluí `admin-menu.constants.js` nessa unificação - o menu lateral (Usuários/Papéis/Configurações) continuava sendo abas via `useState`, sem URL própria. Consequência real: sem link direto pra uma aba, botão Voltar não navegava entre abas, F5 sempre voltava pra "Usuários". Com mais 24 módulos por vir, a auditoria de IA perguntou se isso devia virar rota de verdade agora ou ficar como estava. **Perguntei ao Lucas - decidiu unificar agora.**
...
🟢 **De quebra: 3 melhorias que uma ferramenta de IA pesquisou e o Lucas aprovou implementar**
...
### parte 18 - CRÍTICO: Tailwind saiu do CDN + melhorias no GenericTable - **02-08-2026**

*(Uma sessão de IA rodou o painel de verdade - Postgres+Nest+Vite+navegador, logado como admin - e confirmou as 3 promessas da parte 17 (URL muda por aba, F5 preserva aba, NavLink marca item ativo). Depois foi mais fundo: comparou `dist/index.html` e o CSS gerado contra o que o navegador realmente carrega, e achou um problema sério que só aparece rodando de verdade, nunca só lendo o código.)*

🟢 **CRÍTICO: Tailwind vinha 100% de CDN em runtime - corrigido, agora é dependência real do build**

Achado: `index.html` carregava `<script src="https://cdn.tailwindcss.com">`; a auditoria de IA verificou `dist/` depois do build e confirmou que **nenhuma classe Tailwind existia no bundle** (`bg-primary`/`lg:hidden`/`animate-pulse`: 0 ocorrências, CSS gerado com só 5.3KB) - todo o estilo era gerado em tempo de execução, no navegador, depois de baixar o script do CDN. Sem esse download, zero estilo. Provou com prints reais: sem acesso ao CDN dele, header/footer viraram texto cru, o botão "Menu" (`lg:hidden`) apareceu no desktop, e a gaveta mobile não abria (`translate-x`/`fixed`/`z-30` inexistentes).

Por que importava de verdade, não só estética: a doc oficial do Tailwind diz que `cdn.tailwindcss.com` não é pra produção; rede da escola pode bloquear CDN (o painel abriria quebrado, sem pista de que o bug é rede, não código); dia da banca com wi-fi ruim = painel quebrado na hora H; e tinha FOUC (flash sem estilo) a cada carregamento mesmo com internet boa.

**Corrigido:** instalado `tailwindcss` + `@tailwindcss/vite` (v4.3.3, mesma versão que o CDN já servia) como dependência real - `vite.config.js` ganhou o plugin, `index.html` perdeu o `<script>` do CDN e o `tailwind.config` inline (viraram um bloco `@theme` em CSS, mesmas cores/fontes). Achado próprio nessa migração (não da auditoria de IA): `@import 'tailwindcss'` dentro do mesmo arquivo que já tinha outros `@import url(...)` (dos CSS numerados) quebrava a regra de CSS "import só pode vir antes de qualquer outra regra" - o plugin expande o import inline, empurrando os imports seguintes pra depois de conteúdo real. Resolvido isolando em `assets/css/tailwind-theme.css` próprio, importado separado em `main.jsx`. Confirmado depois do fix: `dist/index.html` sem nenhuma referência a `cdn.tailwindcss.com`; CSS gerado (27KB) contém `bg-primary`, `.lg\:hidden{display:none}` dentro de `@media (width>=64rem)` (equivalente moderno de `min-width:1024px`), `animate-pulse` - tudo real, gerado em build time. Google Fonts/Font Awesome continuam via CDN de propósito (degradam suave: ícone some, fonte cai pro fallback - nada comparável ao Tailwind quebrando a página inteira); self-hostar os dois fica de sugestão em aberto, não crítico.

🟢 **`GenericTable` - 4 melhorias que valem pros 24 módulos futuros, feitas uma vez só aqui**

Todas no componente genérico (não em cada tela), porque cada módulo novo do Nest reaproveita: (1) filtro de texto por linha (aparece só se a tabela já tiver mais de 5 registros - Configurações já tinha 28, achar uma chave exigia rolar tudo); (2) paginação client-side (10 por página, com contador "Página X de Y") - resolve a leitura hoje, mas **não** resolve buscar num universo de milhares sem baixar tudo primeiro (isso exigiria `LIMIT`/`OFFSET` no Nest, fora do escopo desta rodada, registrado aqui pra não esquecer); (3) valor booleano vira badge colorido "Sim"/"Não" em vez do texto cru `true`/`false`; (4) confirmação de exclusão agora aceita um prop `campoRotulo` (ex.: `"nome"`, `"chave"`) pra mostrar um campo legível em vez do ID cru - `window.confirm('Excluir "Admin Sistema"?')` em vez de `'Excluir "8"?'`.

🟢 **Formulário de criar ganhou `<label>` de verdade + campo `tipo` virou `<select>`**

Achado em auditoria de IA: os campos de criar (em Configurações) eram só `placeholder` - some quando a pessoa começa a digitar, perde a referência de qual campo é qual. `camposCriar` (prop do `GenericTable`) ganhou suporte a `tipo: 'select'` + `opcoes: [{valor, rotulo}]` pra campo de conjunto fechado (ex.: `tipo` de configuração: decimal/inteiro/texto/booleano, mesmo `CREATE TYPE tipo_configuracao` do `01_extensoes_enums_tabelas.sql`) - digitar à mão convidava erro de digitação que só o banco ia recusar depois.

🟢 **Aba Papéis: tabela "Papel × Permissão" (~40 linhas repetindo "admin | admin | ...") virou matriz**

`views/2-papel-permissao/matriz-papel-permissao.jsx` - papéis nas colunas, permissões nas linhas, ✓ no cruzamento. Mesma informação, muito menos rolagem, mais fácil de auditar quem tem o quê. Não usa `GenericTable` (formato fundamentalmente diferente - matriz, não lista de linha+ações) e continua só leitura, sem criar/editar/excluir (RBAC gerenciado direto no banco, sem GRANT de escrita pro `app_nestjs` - ver parte 16). As tabelas "Papéis" e "Permissões" (catálogos simples, poucas linhas) não mudaram - a auditoria de IA só sinalizou a de junção como pesada.

**Prova mecânica desta parte 18:** nenhuma mudança no banco (zero `.sql` tocado). `react/` ganhou `tailwindcss` + `@tailwindcss/vite` (v4.3.3, 0 vulnerabilidades). `npm run build` confirmado sem `cdn.tailwindcss.com` em `dist/index.html` e com classes Tailwind reais no CSS gerado (verificação repetida 4 vezes durante a depuração do `@import`, sempre `npm run build`/`npm run lint` limpos ao final de cada etapa). Nenhum teste automatizado no React ainda - comportamento real (filtro, paginação, matriz, formulário com select) não foi visto rodando num navegador de verdade nesta rodada; só compilado e conferido por inspeção do CSS/HTML gerado.

### parte 19 - bug real de cascata (CSS Layers) achado pelo Lucas rodando o painel + "constante" pra botões - **02-08-2026**

*(Lucas testou o painel depois da parte 18 e reportou: cabeçalho/rodapé desalinhados, breadcrumb colado na borda esquerda, botões feios. Junto, pediu uma "constante" pra botão (e outras coisas), pra tirar hardcoded de estilo repetido.)*

🟢 **Bug real: reset próprio (fora de `@layer`) vencia TODO utilitário do Tailwind, sem exceção**

Causa raiz, achada investigando: `1-base.css` tinha `*, *::before, *::after { margin:0; padding:0; box-sizing:border-box }` (e o resto do reset - `body`, `#root`, `h1-h3`, `button`, `input/select/textarea`) **fora de qualquer `@layer`**. Regra da CSS Cascade: estilo sem `@layer` sempre GANHA de qualquer estilo QUE ESTEJA dentro de um `@layer`, não importa a especificidade - e o Tailwind v4 embrulha tudo que gera (`.mx-auto`, `.px-4`, `.py-10`...) dentro de `@layer utilities`. Ou seja, desde a migração do CDN pro build real (parte 18), o `margin:0`/`padding:0` universal estava silenciosamente vencendo `.mx-auto`/`.px-4`/`.py-10` de QUALQUER elemento do projeto inteiro - só não aparecia antes porque o Tailwind CDN injeta `<style>` de um jeito que não usa `@layer` da mesma forma. Sintomas batiam exatamente: header/footer sem `mx-auto` = não centralizavam; breadcrumb sem `px-4` = colado na borda; botões sem padding = feios/espremidos.

**Corrigido:** `1-base.css` (reset), `3-admin-shell.css` (era `2-admin-shell.css`) e `4-crud.css` (era `3-crud.css`) agora vivem dentro de `@layer base`/`@layer components` (mesmos nomes que o Tailwind já declara - CSS junta camadas de mesmo nome na posição certa, antes de `@layer utilities`). Confirmado depois do fix, inspecionando a posição física de cada `@layer` no CSS gerado: `base` (3340) → `components` (7536) → `utilities` (12191), nessa ordem - `.mx-auto` cai dentro de `utilities`, a camada que vence por último. `npm run build`/`lint` limpos.

🟢 **"Constante pra botões" - `2-componentes.css` novo, `@layer components`**

Pedido do Lucas: parar de repetir string gigante de utilitário Tailwind em cada botão - mesma lógica de "não hardcoded" já aplicada no banco (`configuracoes` em vez de número solto). Criadas classes reaproveitáveis: `.btn`/`.btn-primary`/`.btn-secondary`/`.btn-danger` (botões), `.input-padrao` (campo de formulário fora da tabela - login, criar usuário), `.badge`/`.badge-sucesso`/`.badge-neutro` (o Sim/Não booleano do `GenericTable`, que antes vinha com a cor solta na hora - virou classe fixa). Aplicado em: todos os botões do `GenericTable` (Salvar=primary, Cancelar/Editar/Anterior/Próxima=secondary, Excluir=danger - separação nova: excluir agora visualmente diferente das outras ações, não pedido explicitamente mas barato e mais seguro), os links "Criar" das listagens, e os 3 inputs de `criar-usuario.jsx` + os 2 de `login-page.jsx` (mesmo valor visual de antes - só ajustei `.input-padrao` pra `padding:0.875rem` exato, batendo com o `p-3.5` original do Tailwind, zero diferença visual).

**Decisão consciente de não mexer em tudo:** Header/Footer/o botão principal da LoginPage continuam com a string de utilitário Tailwind original, sem trocar pra `.btn` - são "cópia fiel" do Projeto de Interface real (instrução explícita de rodadas anteriores), e ali a fidelidade class-por-class com o HTML original importa mais do que reduzir duplicação. `.btn`/`.input-padrao` valem pra tudo que É desenho próprio (painel admin, telas de CRUD) - não pro que é cópia do protótipo de referência.

**Prova mecânica desta parte 19:** nenhuma mudança no banco. CSS renumerado: `1-base.css` (mesmo lugar) → `2-componentes.css` (NOVO) → `3-admin-shell.css` (era 2) → `4-crud.css` (era 3) → `5-responsividade.css` (era 4), `0-style.css` atualizado. `npm run build`/`npm run lint` confirmados limpos depois do fix de `@layer` e depois da consolidação de botões/inputs - 2 rodadas de build, ambas limpas. Comportamento visual real (cabeçalho centralizado, breadcrumb com respiro, botões com aparência correta) não foi visto num navegador nesta rodada - confirmado só pela ordem física das `@layer` no CSS gerado (base < components < utilities), não por inspeção visual ao vivo.

### parte 20 - CRÍTICO: `try/catch` em volta de uma query fazia login não persistir NADA, silenciosamente - achado em teste de IA, testando o cadastro/sessões de ponta a ponta - **09-08-2026**

*(Contexto: implementando o Bloco B/C de uma rodada de revisão por IA - `listar_papeis_usuario()`, nova função `SECURITY DEFINER` - precisei que o login continuasse funcionando mesmo que essa função ainda não existisse no banco de produção (só existe depois de alguém colar `ATUALIZAR O SUPABASE.sql` no SQL Editor, ver item 22). A "solução" óbvia - `try { ... } catch { return [] }` em volta da chamada - quebrou o login inteiro, de um jeito que NENHUM teste manual via `curl` pegava sozinho: a resposta HTTP vinha 200, com dado que parecia certo.)*

🟢 **Bug real: `try/catch` em JavaScript não desfaz uma transação Postgres abortada - login voltava 200 mas não gravava nada**

Causa raiz: `GlobalDbInterceptor` (commons/database) abre UMA transação por requisição (`BEGIN` → roda o handler inteiro → `COMMIT`/`ROLLBACK`). Quando `listar_papeis_usuario()` não existe no banco, a query lança um erro de Postgres - e isso deixa a TRANSAÇÃO INTEIRA em estado "abortada" até um `ROLLBACK` de verdade acontecer. Pegar essa exceção com `try/catch` do lado do Node e seguir chamando mais coisa no MESMO `db` não desfaz esse estado - as chamadas seguintes (`usuarioServiceFindOne`, montar a resposta) continuavam "funcionando" sem lançar erro visível, e a resposta HTTP voltava 200 com `ultimoLoginEm` fresquinho (lido de DENTRO da transação, antes dela abortar). Só na hora do `COMMIT` final do interceptor é que o problema aparecia - só que de um jeito invisível: Postgres, ao receber `COMMIT` numa transação já abortada, silenciosamente EXECUTA UM ROLLBACK no lugar (é o comportamento documentado do protocolo). Resultado: TUDO que aquela requisição tinha gravado - `registrar_login_sucesso()` (`ultimo_login_em`), a linha nova em `sessao` (o login inteiro) - sumia, sem nenhum erro em lugar nenhum. `curl` sozinho nunca pegava isso (a resposta parecia perfeita); só apareceu rodando `GET /auth/sessoes` logo depois de um login e vendo a lista vazia, e só foi confirmado de verdade instrumentando o próprio interceptor com log manual pra ver o `COMMIT` virando `ROLLBACK` na prática.

**Corrigido:** a chamada arriscada (`listar_papeis_usuario()`) agora fica entre `SAVEPOINT sp_listar_papeis` / `ROLLBACK TO SAVEPOINT sp_listar_papeis` (SQL cru, não `db.transaction()` do Kysely - o dialect customizado deste projeto não implementa savepoint, ver comentário em `kysely-single-connection.dialect.ts`) - se a função não existir, só aquele pedacinho desfaz, a transação de fora continua saudável e o `COMMIT` final funciona de verdade. **Regra geral documentada em `kysely-single-connection.dialect.ts`:** qualquer código que precise "tentar algo e continuar mesmo se falhar" dentro de uma mesma requisição PRECISA de `SAVEPOINT`, nunca um `try/catch` pelado em volta de uma query - vale pra qualquer service futuro, não só este.

**Verificado ao vivo, ponta a ponta, depois do fix:** login → `ultimo_login_em`/`sessao` persistindo de verdade (conferido via conexão separada, não só a resposta HTTP); `GET /auth/sessoes` listando e marcando a sessão atual certa (via claim `sid` novo no JWT, ver `usuario-autenticado.interface.ts`); `DELETE /auth/sessoes` encerrando todas as outras sem tocar a atual; troca de senha exigindo a atual (`senhaAtual`) rejeitando senha errada e aceitando a certa. Conta de teste usada (`fernanda.souza@gmail.com`, seed) restaurada pro nome/senha originais depois do teste.

**Continuação, mesmo dia - 2ª rodada de quebras pelo mesmo motivo, achadas testando o Bloco G (moderação/suspensão):** depois de escrever o `.sql` do Bloco G (colunas `usuario.suspenso_ate`/`motivo_suspensao`/`suspenso_por` e `usuario_papel.suspenso_ate`, ainda NÃO aplicadas no banco de produção - dependem do mesmo `ATUALIZAR O SUPABASE.sql` colado no SQL Editor), o código do NestJS já foi escrito assumindo que essas colunas existiam. Resultado: `POST /auth/login` (checagem de conta suspensa), `GET /usuario/:id/suspensao`, `GET /usuario-papel/:idUsuario` e `GET /usuario-papel` (geral - usado na coluna "papel" da listagem de Usuários) todos quebravam com 500, **confirmado ao vivo antes de perceber e corrigir** - login geral chegou a ficar fora do ar de novo por alguns minutos. Mesmo fix das 4: `SAVEPOINT` antes da query arriscada, `ROLLBACK TO SAVEPOINT` + valor default (`suspensoAte: null`, ninguém suspenso) se a coluna não existir, em vez de deixar o erro subir. Fica registrado aqui como reforço do "regra geral" do parágrafo acima - mesmo já sabendo da regra, escrevi código novo que a violou de novo, na mesma sessão, minutos depois de corrigir a 1ª vez. Vale um lembrete forte: **toda vez que código do NestJS referenciar uma coluna/função que só existe depois de uma migração pendente no SQL Editor, a query correspondente PRECISA de SAVEPOINT + fallback, sem exceção - mesmo em endpoints que "só" leem.**

---

### 🟢 RESOLVIDO no backend (05-09-2026): `GET /configuracoes` agora distingue pública de interna - frontend fica pra rodada seguinte

Isso já era assim desde a criação do endpoint (decisão original documentada no comentário de `configuracao.service.findall.ts`: "Anônimo só enxerga as globais") - não era uma regressão nova, era uma lacuna nunca fechada. Analisando o Projeto de Interface, tinha ficado claro que algumas chaves fazem sentido expor amplamente (`taxa_plataforma_padrao`, `valor_minimo_contribuicao`, `prazo_minimo_campanha_dias`) enquanto outras são mais internas (`limite_tentativas_login`, `bloqueio_login_minutos`) - todas saíam juntas no mesmo `GET`, sem nenhum controle além de "é global ou pessoal". *(Nota: o exemplo original citava `limite_caracteres_relato_denuncia` como interna - a decisão final tratou os limites de caracteres como públicos, ver critério abaixo; exemplo desatualizado, não a decisão.)*

**Resolvido, via discussão com apoio de IA (05-09-2026):** coluna `configuracoes.publica boolean NOT NULL DEFAULT FALSE` - a opção mais simples das cotadas, e a que o `useConfiguracoes()` do React já suportava sem mudar nada (só troca o que `GET /configuracoes` devolve pra quem não tem `configuracao_gerenciar`). Critério de semeadura: **pública** é o que o navegador precisa pra montar/validar uma tela (limites, prazos, tetos - inclusive os `limite_caracteres_*`, que servem de `maxlength`/contador em formulário); **interna** é parâmetro de segurança/moderação (tudo que envolve tentativa de login, bloqueio, validade de token/sessão) ou constante que só uma trigger/service interno lê. Detalhamento completo, incluindo a lista das chaves classificadas uma a uma, em `DOCUMENTACAO_BD.md` (`[01-C]` e `[04-C-1]`). *(Nota 05-09-2026: a Alexia removeu `avatar_padrao_chave` do seed no mesmo dia - ver "Commit da Alexia" logo abaixo - por isso a contagem exata de chaves em `[01-C]` é 38, não mais 37.)*

**Escopo desta rodada, de propósito: só backend** (coluna + policy + seed + CRUD do painel). 🟢 **Frontend, a parte que já era config, feita no mesmo dia:** `seletor-foto-perfil.jsx` trocou o teto de imagem fixo por `obterConfiguracao('arquivo_tamanho_maximo_imagem_bytes', ...)` - ver `ACHADOS_PARA_DISCUTIR.md`, item 5. O que **não** é config (MIME, perfis de redução) continua duplicado à mão - depende da decisão maior de TS, item 10 logo abaixo.

---

### 🟡 Commit da Alexia (05-09-2026, `3fc7a13` "a") - simplificação real do avatar padrão + um bug que ela não viu

Ela removeu de propósito o mecanismo de "avatar padrão do sistema" (uma imagem configurável em `configuracoes.avatar_padrao_chave`, usada como substituta quando o usuário não tinha foto). Raciocínio dela, correto: `AvatarUsuario` já desenha iniciais com fundo colorido quando não há foto, então manter também uma imagem padrão hospedada era complexidade duplicada resolvendo o mesmo problema. Tocou 5 arquivos: removeu a linha de seed, a constante/lookup em `ArquivoServiceResolverAvatar` (incluindo o campo `padrao: boolean` da resposta, que virou só `{ url }`), o comentário/texto em `arquivo.api.js`, e o texto em `criar-usuario.jsx`.

**O que ela não terminou:** `consultar-usuario.jsx` continuava com duas checagens `avatar?.padrao === false` que ela não atualizou. Como o backend não devolve mais o campo `padrao`, essas duas condições ficaram permanentemente falsas (`undefined === false` é sempre `false`), quebrando dois pontos: o selo de "ver foto" sobre o avatar parava de aparecer pra QUALQUER usuário (mesmo quem tem foto de verdade), e o campo "Foto de perfil" na ficha de consulta passava a mostrar sempre "Sem foto (usa iniciais)", mesmo pra quem tinha foto cadastrada - informação errada exibida ao admin. **Corrigido em 06-09-2026, ao puxar este commit pro `main`** - as duas checagens trocadas por `avatar?.url` puro, já que `padrao` não existe mais em lugar nenhum da resposta.

**Achado à parte, menor:** o commit reintroduziu o caractere "—" (travessão) em 12 linhas de comentário novo/editado - contraria a regra do projeto (fixada 04-09-2026, mass-fix de 336 arquivos/2679 ocorrências). Provavelmente ela não tinha conhecimento da convenção (ou o editor dela substitui automaticamente); vale avisá-la, sem necessariamente corrigir essas 12 linhas específicas agora (baixo risco, é só estilo de comentário). **Sugestão de uma IA, melhor que depender de avisar de viva voz (o Lucas vê a Alexia com pouca frequência):** resolver isso mecanicamente com uma regra de lint/hook que reprove o caractere - a convenção passa a se aplicar sozinha em quem quer que escreva, sem precisar que alguém lembre de avisar. Ainda não implementado - depende do Lucas decidir se quer esse hook.

**🔴 Achado por uma IA, revisando este handoff (06-09-2026), CONFIRMADO no código: `avatar_padrao_chave` provavelmente está órfã em produção.** `07_seed_dados.sql` só roda em banco NOVO - o Supabase de produção do Lucas já tinha essa linha inserida desde 24-08-2026 (bloco "módulo 25-arquivo implementado" em `ATUALIZAR O SUPABASE.sql`), e nada além do próprio seed a remove. Removê-la só do arquivo de seed não apaga a linha do banco já provisionado - ela continua lá, sem nenhum código lendo mais ela, aparecendo na tela Configurações do painel como se fosse um parâmetro real. **Corrigido (06-09-2026):** `DELETE FROM configuracoes WHERE chave = 'avatar_padrao_chave';` adicionado em `ATUALIZAR O SUPABASE.sql` **e já rodado pelo Lucas no SQL Editor do Supabase** - produção e seed agora batem (38 chaves nos dois). Também removida a entrada órfã correspondente em `configuracao-grupos.js` (React), que ainda mapeava essa chave morta pro grupo "Arquivo".

---

### 🟢 RESOLVIDO no mesmo dia (05-09-2026): RF-057 não era cumprido de verdade - `encerrar_campanhas_vencidas()` existia no banco, mas nada a chamava

Achado numa revisão de sistema completa (05-09-2026), cruzando os Requisitos Funcionais contra o código de verdade. RF-057 exige que campanhas com prazo vencido sejam encerradas automaticamente (status `sucesso`/`nao_atingida`, bloqueando novas contribuições). A função `encerrar_campanhas_vencidas()` (`05_regras_negocio.sql`) já existia, correta e testada - `SECURITY DEFINER`, bypassa a RLS certinho, o comentário do próprio `.sql` até dizia "Chamada por agendamento (`@Cron` no NestJS)". **O problema: esse `@Cron` nunca tinha sido construído.** Confirmado por grep no projeto inteiro na hora do achado: `@nestjs/schedule` não estava instalado, nenhum `@Cron`/`SchedulerRegistry` existia em `nest/src`, e nenhum `pg_cron`/`cron.schedule` existia em nenhum dos 8 `.sql`. A função ficava **inatingível** - existia, funcionava se chamada manualmente, mas nada nunca chamava ela. Consequência prática: uma campanha cujo prazo (`data_fim`) já tinha passado continuava com `status='ativo'` pra sempre, e - mais sério - continuava **aceitando contribuição** além do prazo.

**Corrigido no mesmo dia**, a pedido do Lucas ("se é fácil de consertar isto... conserte agora"): `npm install @nestjs/schedule`; `ScheduleModule.forRoot()` registrado uma vez em `AppModule`; `CampanhaServiceEncerrarVencidas` (`nest/src/12-campanha/service/campanha.service.encerrar-vencidas.ts`) novo, com `@Cron('*/15 * * * *')` chamando `SELECT public.encerrar_campanhas_vencidas()`. Injeta `PG_POOL` direto, não `DatabaseService.getDb()` - um job agendado não tem requisição HTTP por trás, então não existe transação do `GlobalDbInterceptor` pra pegar (a função é `SECURITY DEFINER`, não precisa de `app.id_usuario_atual` setado de qualquer forma). Testado ao vivo: app sobe, conecta no Supabase real, sem erro (`tsc --noEmit` e `eslint` também limpos). Documentado em `DOCUMENTACAO_BACKEND.md` (§1.1, §7.4, §15.7b) e `DOCUMENTACAO_BD.md` (entrada da função); `MATRIZ-RASTREABILIDADE-RF.md` atualizada pra ✅ nos dois lados.

> ⭐ **Duas perguntas do Lucas que valem registrar aqui em destaque, porque são exatamente o tipo de coisa que uma banca pergunta** (explicação completa, passo a passo, em `DOCUMENTACAO_BACKEND.md`, §7.4):
>
> **"`PG_POOL` direto é a melhor solução? É *future-proof*?"** É a solução certa pro tamanho do projeto hoje - é o padrão recomendado do próprio NestJS pra "algo que roda sozinho, sem ninguém visitando o site" -, mas tem UMA limitação real: só funciona enquanto o servidor Nest estiver "acordado". Hospedagem gratuita (Render) dorme o serviço sem tráfego, e o `@Cron` dorme junto - nenhuma campanha se perde, só atrasa até o site acordar de novo por qualquer motivo (a função sempre compara contra a hora atual quando roda, então não existe "perder" um encerramento, só atrasar). Mesma categoria de limitação que o projeto já aceita conscientemente em RNF-012 (disponibilidade, hospedagem gratuita) - não é erro de desenho, é característica do ambiente gratuito. Caminho mais robusto, se um dia importar de verdade: a extensão `pg_cron` do Supabase, que roda dentro do próprio banco, independente do Nest estar acordado - não implementado agora por ser infraestrutura nova pra um problema hoje pequeno.
>
> **"Rodar isso a cada 15 minutos vai causar lag ou sobrecarregar o banco?"** Não. Existe um índice pronto exatamente pra essa busca (`idx_campanha_status_data_fim`, em `02_indices.sql`) - o Postgres pula direto pras linhas certas, não lê a tabela inteira. E na prática, na maioria das 96 checagens por dia, nenhuma campanha realmente venceu naquela janela de 15 minutos - o comando não muda nada, e não mudar nada é praticamente instantâneo. 96 chamadas por dia é volume desprezível perto de qualquer tráfego real de usuário, muito longe de qualquer limite do plano gratuito do Supabase.
...
### 🔴 Pendência aberta (08-09-2026): `SecaoModeracaoPesquisador` não passa pelo `chamarERegistrar` - suas ações não aparecem no Registro de Chamadas (T4)

O card de Moderação (Pesquisador) - usado hoje em T1 (Campo de Testes) e pensado pra ser reaproveitado, sem mudança nenhuma, na página real de Alterar Usuário mais adiante - chama `perfilPesquisadorApi` direto (`auth.authFetch`), não o `chamarERegistrar` que o resto do Campo de Testes usa pra toda escrita aparecer em T4. É intencional: é literalmente o mesmo componente que precisa continuar igual se for reaproveitado depois (a página real de Usuário não tem `chamarERegistrar`, esse conceito só existe dentro do Campo de Testes) - mudar isso agora significaria reinventar o card só pra esse uso, contrariando a decisão de "não reinventar a roda" que motivou o card em primeiro lugar.

Fica registrado como uma exceção pequena, consciente, à regra de "tudo em Campo de Testes passa por T4" - não é bug, é uma escolha de reaproveitamento que vale a pena lembrar se um dia o T4 parecer incompleto sem motivo aparente.

---

### 🔴 Pendência aberta (08-09-2026): levar o padrão de modal (Consultar/Alterar) pra Gestão de Usuário de verdade
...
**Atualização 4 (13-09-2026): migração completa do CRUD de Usuário pro Modal - as páginas reais (Criar/Alterar/Consultar/Excluir) foram apagadas.** Pedido do Lucas, mesmo dia: "acho que já podemos apagar as telas do CRUD de Usuário, e fazer a completa migração do Modal" - a decisão que ficou em aberto na Atualização 2 foi tomada. Confirmado com o Lucas antes de mexer: (1) o modal viraria um componente de verdade COMPARTILHADO entre a página real e T1 (não duas cópias divergindo com o tempo); (2) "Criar Usuário" também viraria modal, mesmo sem ter equivalente pronto em T1 pra copiar.

**Arquitetura nova:**
- `views/1-usuario/modal-usuario.tsx` (novo) - exporta `ModalAlterarUsuario`/`ModalConsultarUsuario`/`ModalExcluirUsuario`, extraídos de `bancada-pesquisador.tsx` (T1). Cada um recebe só `idUsuario` e busca os PRÓPRIOS dados ao abrir (nome/perfil/avatar/papéis) - não depende mais de uma linha pré-carregada pelo componente pai, então serve tanto pra T1 (Campo de Testes) quanto pro CRUD real.
- **Achado crítico no meio do caminho:** `chamarERegistrar`/`useCampoTestes()` (usados dentro do modal em T1) só funcionam dentro de `<CampoTestesProvider>`, que só é montado em build de DESENVOLVIMENTO (`App.tsx`: `import.meta.env.DEV ? <CampoTestesProvider>...`) - usar isso no modal compartilhado quebraria a página real em produção (`useCampoTestes()` lança exceção sem o provider). O modal foi reescrito pra usar `auth.authFetch` direto, via `usuarioApi`/`perfilPesquisadorApi`/`usuarioPapelApi`/`papelApi` tipados - mesma exceção consciente já aceita pra `SecaoModeracaoPesquisador`/`SecaoModeracao` (não aparecem no Registro de Chamadas quando usados a partir de T1).
- `views/1-usuario/modal-criar-usuario.tsx` (novo) - `ModalCriarUsuario`, só usado pela página real (T1 nunca precisou criar conta do zero, só "Criar Perfil Pesquisador" pra quem já existe).
- **`GenericTable` ganhou `aoAlterar`/`aoConsultar`/`aoExcluir`** (aditivo - `components/crud/generic-table.tsx`) - quando presente, a ação vira `<button onClick>` (abre modal) em vez de `<Link to={rotaBase+...}>` (navega pra página). As ~10 outras telas que usam `rotaBase` (Papéis, Áreas do Conhecimento, Tipos de Link, Configurações, Motivos de Denúncia) continuam navegando por página, sem NENHUMA mudança de comportamento - só ganham a prop nova se um dia quiserem o mesmo tratamento.
- **`listar-usuarios.tsx` reescrita** - usa os callbacks novos do `GenericTable` em vez de `rotaBase`; `chaveRecarga` (um número incrementado a cada ação de sucesso) força o `useEffect([listar])` interno do `GenericTable` a buscar de novo, já que ele não expõe um "recarregar" próprio de propósito (quem decide QUANDO recarregar é sempre quem usa a tabela, não a tabela genérica).
- **`bancada-pesquisador.tsx` (T1) encolheu de ~1930 pra ~370 linhas** - perdeu toda a lógica de modal (que virou `modal-usuario.tsx`), ficou só com a tabela/filtro/facet/paginação que é exclusiva dela (linha riscada/cadeado por registro bloqueado, que `GenericTable` não sabe fazer).
- **`alterar-usuario.tsx`/`consultar-usuario.tsx`/`criar-usuario.tsx`/`excluir-usuario.tsx` apagados**, junto com as 4 rotas correspondentes em `rotas.constants.ts` (`/admin/usuarios/criar`, `/:id/alterar`, `/:id/consultar`, `/:id/excluir`).

**Achado no meio do caminho, corrigido:** ao extrair o modal, o botão "Gerar CPF válido" (dentro de "Criar Perfil Pesquisador") ficou pelo caminho na primeira versão - `gerarCpfValido()` mora em `services/campo-testes/`, de propósito (só faz sentido testando, nunca faria sentido um admin "gerar um CPF fictício" pra uma pessoa real). Corrigido com uma prop opcional `gerarCpfDeTeste?: () => string` no modal - só T1 passa a função de verdade, a página real nunca vê o botão. Achado numa auditoria de contagem de elementos (comparando o arquivo original, via `git show HEAD:...`, contra os arquivos novos) feita de propósito depois da extração, já que um corte-e-cola desse tamanho tem risco real de esquecer alguma coisa pelo caminho - o resto da auditoria (aria-label a aria-label, texto de botão a texto de botão) não achou mais nada faltando.

Testado ao vivo (Playwright): Consultar/Alterar/Criar/Excluir funcionando na página real (`/admin/usuarios`) e em T1 (`/admin/campo-testes/pesquisador`), os dois batendo no MESMO componente; "Gerar CPF válido" aparecendo só em T1; zero erro de console (um 404 esperado, avatar de conta sem foto). `tsc`/`eslint`/`build` limpos nos dois lados.

**Decisão tomada (mesmo dia, resposta do Lucas):** T1 continua existindo - "embora possua o mesmo Modal, parece sujeira, mas T1 é importante para os testes, principalmente para testar o upgrade de perfil rapidamente, scores, e etc." As duas telas ficam lado a lado, T1 como ferramenta de teste (lista todos os usuários, sem paginação de servidor) e a página real como o CRUD de produção de verdade - não é duplicação de mecanismo, é o mesmo componente reaproveitado em dois contextos diferentes.

**Varredura de código morto/inerte pedida pelo Lucas logo em seguida ("só verifique se não ficou código morto pra trás") - 3 achados:**
- 🟢 **`.crud-tabela__linha--selecionada` (`8-campo-testes.css`) - CSS morto, apagado.** Estilizava a linha destacada do antigo "Escolher" (pesquisador escolhido em T1, campanha em foco em T2) - com os dois mecanismos removidos (12 e 13-09-2026), zero `.tsx` no projeto inteiro ainda referenciava a classe. Removida a regra e o comentário que a explicava.
- 🟢 **Comentário de cabeçalho do mesmo arquivo corrigido.** Dizia "ESTE ARQUIVO EXISTE SOLENEMENTE PARA O CAMPO DE TESTES" - não é mais 100% verdade, já que `.links-academicos-wrapper` (mesmo arquivo) também estiliza `<PainelLinksAcademicos>` dentro do modal compartilhado, usado pela página real. Não vale a pena mover a regra por causa disso - só documentada a exceção.
- 🔴→🟢 **Achado mais sério, corrigido com aprovação do Lucas: T1 tinha parado de aparecer no T4 (Registro de Chamadas) por completo.** O modal compartilhado usa `auth.authFetch` direto (não pode usar `chamarERegistrar`/`useCampoTestes()`, que quebrariam a página real em produção) - consequência real da extração, não cosmética, já que T4 é justamente a ferramenta que deixa ver de perto as ações que T1 existe pra testar. **Corrigido:** cada modal (`ModalAlterarUsuario`/`ModalConsultarUsuario`/`ModalExcluirUsuario`, e os componentes internos `PainelLinksAcademicos`/`PainelScore`) ganhou uma prop opcional `aoRegistrarChamada?: (entrada: EntradaRegistroChamada) => void`; um helper novo (`comRegistro()`) cronometra e reporta cada chamada só quando essa prop existe. A página real nunca passa essa prop (continua sem nenhuma dependência do `CampoTestesProvider`); T1 passa `registrarChamada` de `useCampoTestes()`. Aproximação consciente e documentada: em sucesso, o status reportado é sempre `200` (a camada tipada não expõe o código HTTP real de sucesso, só o corpo já tratado) - em erro, `ErroHttp.status` já carrega o código verdadeiro.

Testado ao vivo (Playwright): T4 foi de 0 pra 8 chamadas depois de abrir só um Consultar em T1, mostrando método/caminho/status/tempo/corpo reais (ex.: `GET /perfil-pesquisador/1/score - 200 - 77ms`); a página real (`/admin/usuarios`) continua sem nenhum T4 na tela, confirmando zero acoplamento. `tsc`/`eslint`/`build` limpos.
