...
🔴 **51. Colisão de letra pré-existente: `[03-F]`/`[03-G]` reaproveitam letras que já pertencem a outros domínios**

Achado em 09-08-2026 escrevendo o Bloco G (moderação/suspensão) — mesma classe de bug já corrigida uma vez neste projeto (`[03-K]`→`[03-M]`), só que mais antiga e ainda não corrigida. O Índice Global de Letras (topo de `DOCUMENTACAO_BD.md`) atribui `F` a LINK (Vinculação de URLs Externas) e `G` a ARQUIVO (Vinculação de Mídias) — mas `03_funcoes_seguranca.sql` tem `[03-F]` ("OPERAÇÕES DE AUTENTICAÇÃO": `confirmar_email_por_token`, `registrar_falha_login`, `liberar_bloqueio_login`, `registrar_login_sucesso`, `excluir_conta_usuario`) e `[03-G]` ("MODERAÇÃO SOBRE PESQUISADOR": `suspender_pesquisador`, `reativar_pesquisador`), nenhum dos dois relacionado a LINK/ARQUIVO. Não corrigido agora — é um rename cruzando comentários e referências já estabelecidas há várias rodadas, fora do escopo de quem só estava adicionando um bloco novo (`[03-N]`, moderação de conta, mesma rodada) e por isso escolheu uma letra livre em vez de piorar a colisão.

> Sugestão do *** CLAUDE ***: quando alguém for mexer de novo em `03_funcoes_seguranca.sql` por outro motivo, aproveitar pra renomear `[03-F]`→` [03-?]` e `[03-G]`→`[03-?]` pra letras livres, atualizando o Índice Global de Letras junto. Não é urgente (não é bug funcional, só de nomenclatura/documentação), mas quanto mais rodadas passarem reaproveitando `F`/`G` sem perceber, mais confuso fica desfazer depois.
