...
🟠 **13. Quatro regras do score que precisam de decisão, caso ele fique** *(atualização 28-07-2026: (a), (b) e um quinto ponto técnico já foram corrigidos — ver "🟢 Já corrigido" mais abaixo. Ficam pendentes (c) e (d), abaixo.)*

Se a decisão do item 12 for manter o score, estes quatro pontos precisam ser resolvidos antes de o mexer em qualquer função de cálculo. Nenhum é bug: são escolhas de regra que hoje estão implementadas de um jeito que talvez não seja o que vocês queriam. (a) 🟢 Denúncia improcedente penaliza igual — a função `calcular_score_reputacao` conta todas as denúncias contra o pesquisador, inclusive as que a moderação julgou `improcedente`, o que contradiz o RF-077, que define improcedente como "denúncia descartada após análise". *(Corrigido — conformidade com RF-077, não decisão de negócio, ver resolvidos.)* (b) 🟢 Campanha rejeitada pune duas vezes — a rejeição derruba a taxa de aprovação e ainda entra no denominador da taxa de conclusão. *(Corrigido — erro aritmético, ver resolvidos.)* (c) 🔴 Campanha encerrada antecipadamente conta como sucesso — `calcular_score_historico` trata o status `encerrado` (o encerramento do RF-040, com justificativa) como se fosse meta atingida. **Ainda pendente — esta é genuinamente uma escolha de vocês dois, não conformidade.** (d) 🔴 GitHub não pontua — a função procura por Lattes, ORCID, LinkedIn, ResearchGate e mais três padrões que não existem na allowlist, mas nenhum padrão casa com GitHub, então um pesquisador com repositório público não ganha nada por isso. **Ainda pendente.**
...
> Confirmação (28-07-2026): (a), (b) e o quinto ponto (consolidar as constantes em `score_config`) não são escolhas de negócio — são conformidade com RF já escrito (a), erro aritmético (b) e limpeza técnica (5º ponto). Implementados e testados: simulei a correção de (a) rodando contra o seed inteiro e nenhum dos 4 pesquisadores desenhados pro teste de faixas muda de faixa (Rafael 44→45, Marcos 33→35, Eduardo 46→48, Vinícius 19→19) — ver "🟢 Já corrigido" mais abaixo. (c) continua sendo escolha de vocês — é a opção conservadora, mas é escolha. (d) fica com o resto das decisões de escopo do score.
...
🟠 **16. Limites de negócio fixos no código em vez de configuráveis** *(atualização 28-07-2026: a parte estrutural — tirar os 4 números do código e passar pra `configuracoes` + trigger — já foi feita, ver "🟢 Já corrigido" mais abaixo. Fica pendente só a decisão de produto: 90 ou 60 dias?)*
...
> **Implementado (28-07-2026):** exatamente esse desenho — constraint virou limite técnico largo (1-365 dias, ver `01`), e uma trigger nova lê `configuracoes.prazo_minimo_campanha_dias`/`prazo_maximo_campanha_dias` (15/90, os mesmos valores de sempre) pra aplicar a regra de negócio real. Os outros 3 limites (2 campanhas, 4 endossos, 5 denúncias/24h) passaram pelo mesmo tratamento, cada um com sua chave nova em `configuracoes`. **Não implementado, e não é técnico:** a escolha entre 90 e 60 dias em si — isso continua sendo decisão de vocês dois; o que mudou é que agora essa decisão custa um `UPDATE`, não uma migração.
...
🔴 **19. Colunas que faltam para requisitos que já estão escritos** *(atualização 28-07-2026: (b) e (d) já foram feitas — ver "🟢 Já corrigido" mais abaixo. Ficam pendentes (a), (c) e (e), abaixo — (a)/(c) são "quase", só falta uma linha sua dizendo se mantém; (e) segue travado, ligado ao item `tipo_link` reaberto.)*

Comparando o banco contra a Etapa 3, existem cinco pontos em que um requisito já aprovado descreve algo que o banco não consegue armazenar. (a) 🔴 Rótulo personalizado de link acadêmico — os RF-014, RF-016 e RF-018 e a Etapa 2 falam em até 5 links com rótulo personalizável, mas `link_academico` não tem coluna de rótulo nem trava de 5. (b) 🟢 Descrição da denúncia — os RF-019 e RF-072 pedem campo opcional de descrição adicional, que não existe na tabela `denuncia`. *(Corrigido: coluna `relato TEXT`, ver resolvidos.)* (c) 🔴 Vídeo de apresentação — o RF-033 pede campo opcional de URL de vídeo exibido em destaque na página da campanha, e `campanha` não tem essa coluna. (d) 🟢 Justificativa do Administrador ao rejeitar encerramento antecipado — o RF-041 torna obrigatório, mas `solicitacao_encerramento` só tem campo para a justificativa do pesquisador. *(Corrigido: coluna `justificativa_admin TEXT`, ver resolvidos — esta era a mais urgente das cinco.)* (e) 🔴 Tipos de link — o RF-014 lista sete tipos, incluindo "Site Institucional" e "Outro" com regras próprias, e o seed tem cinco *(ver também a nota sobre `tipo_link` reaberto, na seção "Pendências mais antigas" abaixo)*.
...
> Nota (28-07-2026): (b) e (d) não são decisão — são conformidade com RF já escrito, implementadas. (a) e (c) continuam sendo "manteria" na minha opinião, mas são custo de produto (coluna nova, e no caso de (a) uma trava de 5 por usuário) — ficam pendentes de uma linha sua confirmando antes de eu mexer.
...
`07_seed_dados.sql` cadastra a configuração `score_minimo_campanha = 25.00` ("Score mínimo para criar campanha"), com um comentário explícito no próprio arquivo:
...
🟠 **`tipo_link` com só 5 de 7 tipos seedados — REABERTO, ligado ao item 19(e) da Lista C**

O comentário do próprio seed (`07_seed_dados.sql`) diz *"tipo_link ajustado para a allowlist fechada definida pela equipe"* — mas essa decisão foi tomada com evidência incompleta: verificou quais tipos foram seedados, mas nunca olhou as 3 colunas de escopo (`permite_perfil`/`permite_atualizacao`/`permite_recompensa`), que ficam todas no valor padrão pros 5 tipos seedados e por isso derrubam duas tabelas inteiras (item 15 da Lista C). A Etapa 3 dos `.docx` (RF-014) parece listar 7 tipos, incluindo "Site Institucional" e "Outro" com comportamento próprio — o que contradiz a "decisão fechada" registrada no comentário do seed. **Reaberto** até vocês dois confirmarem qual foi a decisão de verdade. *(Nota 28-07-2026: `tipo_link` ganhou coluna `codigo` nessa data — ver item 39, na seção de resolvidos — mas isso foi só pra dar chave natural aos 5 tipos que já existem; não resolve nem mexe nesta pendência dos 7 tipos.)*
